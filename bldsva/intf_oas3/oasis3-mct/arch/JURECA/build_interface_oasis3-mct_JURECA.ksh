...
print "${cblue}> setupOas${cnormal}"
  ...
  if [[ $withPFL == "true" && $withCOS == "true" ]] then
    cp $rootdir/bldsva/data_oas3/namcouple_cos_clm_pfl $rundir/namcouple
    ncpl_exe1=$nproc_cos
    ncpl_exe2=$nproc_pfl
    ncpl_exe3=$nproc_clm

    sed "s/nproc_exe1/$nproc_cos/" -i $rundir/namcouple
    sed "s/ncpl_exe1/$ncpl_exe1/" -i $rundir/namcouple
    sed "s/nproc_exe2/$nproc_pfl/" -i $rundir/namcouple
    sed "s/ncpl_exe2/$ncpl_exe2/" -i $rundir/namcouple
    sed "s/nproc_exe3/$nproc_clm/" -i $rundir/namcouple
    sed "s/ncpl_exe3/$ncpl_exe3/" -i $rundir/namcouple
    sed "s/cplfreq1/$cplfreq1/" -i $rundir/namcouple
    sed "s/cplfreq2/$cplfreq2/" -i $rundir/namcouple
    
    sed "s/ngpflx/$gx_pfl/" -i $rundir/namcouple
    sed "s/ngpfly/$gy_pfl/" -i $rundir/namcouple
    sed "s/ngclmx/$gx_clm/" -i $rundir/namcouple
    sed "s/ngclmy/$gy_clm/" -i $rundir/namcouple
    sed "s/ngcosx/$gx_cos/" -i $rundir/namcouple
    sed "s/ngcosy/$gy_cos/" -i $rundir/namcouple

  fi  

  if [[ $withPFL == "true" && $withCOS == "false" ]] then
    cp $rootdir/bldsva/data_oas3/namcouple_pfl_clm $rundir/namcouple
    ncpl_exe1=$nproc_pfl
    ncpl_exe2=$nproc_clm

    sed "s/nproc_exe1/$nproc_pfl/" -i $rundir/namcouple
    sed "s/ncpl_exe1/$ncpl_exe1/" -i $rundir/namcouple
    sed "s/nproc_exe2/$nproc_clm/" -i $rundir/namcouple
    sed "s/ncpl_exe2/$ncpl_exe2/" -i $rundir/namcouple
    sed "s/cplfreq2/$cplfreq2/" -i $rundir/namcouple
  ...
  if [[ $withPFL == "false" && $withCOS == "true" ]] then
    cp $rootdir/bldsva/data_oas3/namcouple_cos_clm $rundir/namcouple
    ncpl_exe1=$nproc_cos
    ncpl_exe2=$nproc_clm
    sed "s/nproc_exe1/$nproc_cos/" -i $rundir/namcouple
    sed "s/ncpl_exe1/$ncpl_exe1/" -i $rundir/namcouple
    ...
    sed "s/totalruntime/$(($runhours*3600))/" -i $rundir/namcouple
    sed "s/yyyymmdd/${yyyy}${mm}${dd}/" -i $rundir/namcouple

    sed "s@$gx_cos $gy_cos@$(($gx_cos - 8)) $(($gy_cos - 8))@g" -i $rundir/namcouple
print "${cblue}< setupOas${cnormal}"
}
