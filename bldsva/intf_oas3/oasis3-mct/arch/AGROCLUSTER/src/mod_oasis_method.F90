...
!----------------------------------------------------------------------
   SUBROUTINE oasis_get_freq(fieldid,cplfreq, kinfo)
        ...
        INTEGER (kind=ip_intwp_p),intent(in)    :: fieldid
        INTEGER (kind=ip_intwp_p),intent(out)    :: cplfreq
        INTEGER (kind=ip_intwp_p),intent(inout),optional    :: kinfo
        character(len=*),parameter :: subname = 'oasis_get_freq'
!  ---------------------------------------------------------
        ...
        if (fieldid .le.  nnamcpl) then
                cplfreq = namflddti(fieldid) 
        else
