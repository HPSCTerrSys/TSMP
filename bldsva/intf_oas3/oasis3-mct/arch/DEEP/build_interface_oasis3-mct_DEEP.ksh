...
route "${cyellow}>> substitutions_oas${cnormal}"
  ...
  comment "   mv all m_* to ${prefix}m_*"
    if [ -e m_Transfer.F90 ]; then for i in $(ls m_*) ; do mv $i ${prefix}${i}  >> $log_file 2>> $err_file ; check ; done; fi
  comment "   mv mct_mod.F90 to ${prefix}mct_mod.F90"
    if [ -e mct_mod.F90 ]; then  mv -f mct_mod.F90 ${prefix}mct_mod.F90; fi >> $log_file 2>> $err_file
  check
...
route "${cyellow}>> configure_oas${cnormal}"
  ...
  comment "   cp DEEP oasis3-mct makefile to /util/make_dir/"
    cp $rootdir/bldsva/intf_oas3/oasis3-mct/arch/$platform/config/make.intel_DEEP_oa3 $file >> $log_file 2>> $err_file
  check
  c_configure_oas
