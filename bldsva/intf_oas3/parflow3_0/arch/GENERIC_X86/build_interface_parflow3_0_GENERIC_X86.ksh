...
route "${cyellow}>> configure_pfl${cnormal}"
  ...
  check
    ...
    libsSim="$cplLib -lmpich -L$ncdfPath/lib -lnetcdff"
  ...
  comment "   sed correct linker command in pfsimulator"
    sed "s/inline double VanGLookupSpline/double VanGLookupSpline/g" -i $pfldir/pfsimulator/parflow_lib/problem_phase_rel_perm.c >> $log_file 2>> $err_file
  check
  comment "   sed correct linker command in pfsimulator"
