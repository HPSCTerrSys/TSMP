...
contains
  ...
  subroutine cime_pre_init1(esmf_log_option)
    ...
    IF (oas_error /= 0) &
      CALL oasis_abort(oas_comp_id, 'oas_clm_init', 'Failure in oasis_init')
    CALL oasis_get_localcomm(global_comm, oas_error)
    IF (oas_error /= 0) &
      CALL oasis_abort(oas_comp_id, 'oas_clm_init', 'Failure in oasis_get_localcomm')
#endif
