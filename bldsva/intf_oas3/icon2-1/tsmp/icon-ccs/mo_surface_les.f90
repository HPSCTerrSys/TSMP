...
MODULE mo_surface_les
  ...
  SUBROUTINE  surface_conditions(p_nh_metrics, p_patch, p_nh_diag, p_int, &
    ...
    REAL(wp) :: ustar_v(nproma,p_patch%nblks_c)         ! SBr, CHa
    ...
    CASE(1)
     ...
     ! SBr, CHa
     ustar_v(:,:) = SQRT((prm_diag%umfl_s**2 + prm_diag%vmfl_s**2))
     p_nh_metrics%obukhov_length(:,:) = -theta(:,nlev,:)*ustar_v(:,:)**3/(akt*grav*prm_diag%shfl_s(:,:))
