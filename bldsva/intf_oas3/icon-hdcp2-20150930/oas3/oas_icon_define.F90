...
INTEGER         :: ji, jj, jg
INTEGER         :: jg_end, jh_beg     ! index
INTEGER         :: NULOUT=6
INTEGER         :: id_part    !
INTEGER        :: write_aux_files    !
INTEGER         :: paral(5)       ! OASIS3 box partition
INTEGER         :: var_nodims(2)       ! OASIS3 box partition
...
INTEGER, DIMENSION(ie_tot,je_tot) :: imask_tot
...
INTEGER        :: nlei_tot, nlej_tot  ! upper halo limits on a global subdomain
INTEGER         :: jih_tot, jjh_tot    ! global subdomain size without halo
...
!------------------------------------------------------------------------------
!- Begin Subroutine oas_icon_define 
!------------------------------------------------------------------------------
 ...
 IF ( nerror > 0 ) THEN
   CALL prism_abort_proto(ncomp_id,'oas_icon_define','Failure in allocating zmask')
   RETURN
 ENDIF
 ...
 IF ( COUNT( zmask(nldi:nlei, nldj:nlej) < 0.5) < 1 ) THEN
   WRITE(NULOUT,*) ' oas_icon_define : All PE grid points masked '
   CALL FLUSH(NULOUT)
   lpe_cpl = .FALSE.
 ELSE
   lpe_cpl = .TRUE.
...
!! interpolation between the COSMO grid and CLM pseduo-atmosphere grid.
  ...
  IF (my_cart_id == 0) THEN
     ...
     WRITE(nulout,*) "oasicon: oas_icon_define: Reading clmgrid ....."
...
!CPS                                 zlat_tot(nldi:nlei_tot, nldj:nlej_tot)*0.+1.)
     ...
     IF ( IOASISDEBUGLVL > 0 ) THEN
        WRITE(nulout,*) " oasicon: oas_icon_define: prism_terminate_grids_writing"
        CALL flush(nulout)
     ENDIF
...
!CPS      IF ( write_aux_files == 1 ) call MPI_Barrier(mpi_comm, nerror)
 ...
 IF ( lpe_cpl ) THEN
   ...
   IF ( nerror > 0 ) THEN
     CALL prism_abort_proto( ncomp_id, 'oas_icon_define', 'Failure in allocating exfld' )
     RETURN
   ENDIF
   ...
   IF( nerror /= PRISM_Success )   CALL prism_abort_proto (ncomp_id, 'oas_icon_define',   &
        &                                           'Failure in prism_def_partition' )

   IF ( IOASISDEBUGLVL > 0 ) THEN
     WRITE(nulout,*) ' oasicon: oas_icon_define: prism_def_partition '
   ENDIF
   ! -----------------------------------------------------------------
   ! ... Variable definition
   ! ----------------------------------------------------------------
...
!    ksnd = 16                 !CPS now defined in oas_icon_vardef
...
!     PRINT*, 'ERROR (oas_icon_define): ssnd(17)%laction has to be .TRUE.'
 ...
 END IF
   ...
   DO ji = 1, nmaxfld
     IF ( ssnd(ji)%laction ) THEN 
        ...
        IF ( nerror /= PRISM_Success )   CALL prism_abort_proto( ssnd(ji)%nid, 'oas_icon_define',   &
...
!CPS    krcv = 0
   DO ji = 1, nmaxfld
     IF ( srcv(ji)%laction ) THEN 
        ...
        IF ( nerror /= PRISM_Success )   CALL prism_abort_proto( srcv(ji)%nid, 'oas_icon_define',   &
...
!CPS            krcv = krcv + 1
   ...
   IF ( nerror /= PRISM_Success )   CALL prism_abort_proto ( ncomp_id, 'oas_icon_define', 'Failure in prism_enddef')

 ENDIF        ! lpe_cpl 
     
 IF ( IOASISDEBUGLVL > 0 ) THEN 
    WRITE(nulout,*) "oasicon: prism_enddef"
 ENDIF
 ...
 IF ( IOASISDEBUGLVL >= 0 ) THEN
    WRITE(nulout,*) "oasicon: coupling frequency :", cplfreq
 ENDIF
...
END SUBROUTINE oas_icon_define
