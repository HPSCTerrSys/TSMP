...
!---------------------------------------------------------------------
! Description:
!  This routine sends the fluxes from CLM3.5 to ICON
!
! References:
...
!
!   This gives 2 options for coupling ICON and CLM i.e either flux or transfer coefficients
...
REAL(KIND=r8), POINTER  :: snd_tmp(:)                 !CPS General variable for 1D arrays
REAL(KIND=r8), POINTER  :: snd_tmp2(:,:)              !CPS for Albedo
REAL(KIND=r8), POINTER  :: albt_gcell(:,:)            !CPS for Albedo

REAL(KIND=r8), ALLOCATABLE      :: snd_field(:,:)
REAL(KIND=r8), ALLOCATABLE      ::   fsnd(:,:,:)      ! temporary arrays

REAL(KIND=r8)                   :: toc, tic

INTEGER                         :: status, ncid, ncvarid(ksnd), dimids(3) !CPS Debug Outputs
INTEGER                         :: ctr     !CPS
...
!CPS WRITE(6,*), "oasclm: send routine ", numg, numl, numc, nump

 ! allocate global clm buffers
 IF (masterproc) THEN 
   IF (.NOT. associated(snd_tmp)) THEN 
     ALLOCATE(snd_tmp(numg), stat=ier)
     IF (ier /= 0) THEN 
        CALL prism_abort_proto( ncomp_id, 'send_fld_2cos', 'Failure in allocating snd_tmp' )
        RETURN
     ENDIF 
     ALLOCATE(snd_field(numg,ksnd), stat=ier)
     IF (ier /= 0) THEN
        CALL prism_abort_proto( ncomp_id, 'send_fld_2cos', 'Failure in allocating snd_field' )
        RETURN
     ENDIF
     ALLOCATE(snd_tmp2(2,numg), stat=ier)     !Needed for sending albedo
     IF (ier /= 0) THEN 
        CALL prism_abort_proto( ncomp_id, 'send_fld_2cos', 'Failure in allocating snd_tmp2' )
        RETURN
     ENDIF
   ENDIF 
 ENDIF 

 ALLOCATE(albt_gcell(2,begg:endg), stat=ier)     !Needed to transpose Albedo at local level
 IF (ier /= 0) THEN
    CALL prism_abort_proto( ncomp_id, 'send_fld_2cos', 'Failure in allocating snd_tmp2' )
    RETURN
 ENDIF
...
!CPS WRITE(6,*) "oasclm: send_fld_2cos: p2g done"   
 ...
 IF (masterproc) toc=MPI_Wtime() 

 ! Create remapping matrix between land and atmosphere model
 CALL clm_mapl2a(clm_l2a, atm_l2a)

!CPS WRITE(6,*) "oasclm: send_fld_2cos: remapping matrix done"

!CPS WRITE(6,*) "oasclm: send_fld_2cos: gather data to masterproc"

 ! retrieve clm3.5 surface fluxes from all nodes
 CALL gather_data_to_master(atm_l2a%taux, snd_tmp, clmlevel=nameg)
 IF (masterproc) snd_field(:,jps_taux)=snd_tmp
 CALL gather_data_to_master(atm_l2a%tauy, snd_tmp, clmlevel=nameg)
 IF (masterproc) snd_field(:,jps_tauy)=snd_tmp
 CALL gather_data_to_master(atm_l2a%eflx_lh_tot, snd_tmp, clmlevel=nameg)
 IF (masterproc) snd_field(:,jps_lat)=snd_tmp
 CALL gather_data_to_master(atm_l2a%eflx_sh_tot, snd_tmp, clmlevel=nameg)
 IF (masterproc) snd_field(:,jps_sens)=snd_tmp
 CALL gather_data_to_master(atm_l2a%eflx_lwrad_out, snd_tmp, clmlevel=nameg)
 IF (masterproc) snd_field(:,jps_ir)=snd_tmp
!CPS Albedo 
 albt_gcell = TRANSPOSE(atm_l2a%albd)
 CALL gather_data_to_master(albt_gcell, snd_tmp2, clmlevel=nameg)
 IF (masterproc) snd_field(:,jps_albd)= 0.5_r8*(snd_tmp2(1,:)+snd_tmp2(2,:))
 albt_gcell = TRANSPOSE(atm_l2a%albi)
 CALL gather_data_to_master(albt_gcell, snd_tmp2, clmlevel=nameg)
 IF (masterproc) snd_field(:,jps_albi)= 0.5_r8*(snd_tmp2(1,:)+snd_tmp2(2,:))
!CPS Albedo

#ifdef COUP_OAS_ICON
 CALL gather_data_to_master(atm_l2a%nee, snd_tmp, clmlevel=nameg)   !CPS
 IF (masterproc) snd_field(:,jps_co2fl)=snd_tmp
 CALL gather_data_to_master(atm_l2a%ram1, snd_tmp, clmlevel=nameg)   !CPS
 IF (masterproc) snd_field(:,jps_ram1)=snd_tmp  
 CALL gather_data_to_master(atm_l2a%rah1, snd_tmp, clmlevel=nameg)   !CPS
 IF (masterproc) snd_field(:,jps_rah1)=snd_tmp
 CALL gather_data_to_master(atm_l2a%raw1, snd_tmp, clmlevel=nameg)   !CPS
 IF (masterproc) snd_field(:,jps_raw1)=snd_tmp
 CALL gather_data_to_master(atm_l2a%t_sf, snd_tmp, clmlevel=nameg)   !CPS
 IF (masterproc) snd_field(:,jps_tsf1)=snd_tmp
 CALL gather_data_to_master(atm_l2a%q_sf, snd_tmp, clmlevel=nameg)   !CPS
 IF (masterproc) snd_field(:,jps_qsf1)=snd_tmp
!MU (12.04.13)
 CALL gather_data_to_master(atm_l2a%fpsn, snd_tmp, clmlevel=nameg)
 IF (masterproc) snd_field(:,jps_fpsn)=snd_tmp
 CALL gather_data_to_master(atm_l2a%fplres, snd_tmp, clmlevel=nameg)
 IF (masterproc) snd_field(:,jps_fplres)=snd_tmp
!MU (12.04.13)
 ...
 IF (masterproc) THEN                  ! masterproc
   tic=MPI_Wtime()
   IF ( IOASISDEBUGLVL == 2 ) &
     WRITE(6, *)  'oasclm: send_fld_2cos: gather time ', tic-toc

   ji = adecomp%gdc2i(numg)
   jj = adecomp%gdc2j(numg)

   ALLOCATE ( fsnd(ndlon,ndlat,ksnd), stat=ier)  
   IF (ier /= 0) THEN 
  ...
  !CPS Debug Ouput of l2a fields on Atm Grid
  IF ( IOASISDEBUGLVL == 1) THEN         

    IF (isec == 0) THEN
       status = nf90_create("debugsnd_clm_cos.nc", NF90_CLOBBER, ncid)
       status = nf90_def_dim(ncid, "x", ndlon, dimids(1))
       status = nf90_def_dim(ncid, "y", ndlat, dimids(2))
       status = nf90_def_dim(ncid, "t", 48 , dimids(3))         !HARDWIRED
       ctr = 0
       DO jn = 1, ksnd
         IF (ssnd(jn)%laction) THEN
           ctr = ctr+1
           status = nf90_def_var(ncid, ssnd(jn)%clname, NF90_DOUBLE, dimids, ncvarid(ctr))
         ENDIF
       ENDDO
    ...
    ENDIF                    ! isec = 0
       ...
       status = nf90_open("debugsnd_clm_cos.nc", NF90_WRITE, ncid)
       ctr = 0
       DO jn = 1,ksnd
         IF (ssnd(jn)%laction) THEN
           ctr = ctr+1
           status = nf90_inq_varid(ncid, ssnd(jn)%clname , ncvarid(ctr))
!CPS           WRITE(6, *) "CPS CLM2ICON " ,jn, get_nstep(), MINVAL(fsnd(:,:,jn)),MAXVAL(fsnd(:,:,jn))
           status = nf90_put_var( ncid, ncvarid(ctr), fsnd(:,:,jn),        &
                              start = (/ 1, 1, get_nstep()/),              &
                              count = (/ ndlon, ndlat, 1 /) )
         ENDIF
       ENDDO
  ...
  !Start sending of CLM3.5 variables

  IF( ssnd(jps_taux)%laction )  CALL oas_clm_snd( jps_taux, isec, fsnd(:,:,jps_taux), info )
  IF( ssnd(jps_tauy)%laction )  CALL oas_clm_snd( jps_tauy, isec, fsnd(:,:,jps_tauy), info )
  IF( ssnd(jps_lat)%laction )  CALL oas_clm_snd( jps_lat, isec, fsnd(:,:,jps_lat), info )
  IF( ssnd(jps_sens)%laction )  CALL oas_clm_snd( jps_sens, isec, fsnd(:,:,jps_sens), info )
  IF( ssnd(jps_ir)%laction )  CALL oas_clm_snd( jps_ir, isec, fsnd(:,:,jps_ir), info )
  IF( ssnd(jps_albd)%laction )  CALL oas_clm_snd( jps_albd, isec, fsnd(:,:,jps_albd), info )
  IF( ssnd(jps_albi)%laction )  CALL oas_clm_snd( jps_albi, isec, fsnd(:,:,jps_albi), info )
!MU (18.09.2012)
  IF( ssnd(jps_co2fl)%laction )  CALL oas_clm_snd( jps_co2fl, isec, fsnd(:,:,jps_co2fl), info )
!MU (18.09.2012)
  IF( ssnd(jps_ram1)%laction )  CALL oas_clm_snd( jps_ram1, isec, fsnd(:,:,jps_ram1), info )
  IF( ssnd(jps_rah1)%laction )  CALL oas_clm_snd( jps_rah1, isec, fsnd(:,:,jps_rah1), info )
  IF( ssnd(jps_raw1)%laction )  CALL oas_clm_snd( jps_raw1, isec, fsnd(:,:,jps_raw1), info )
  IF( ssnd(jps_tsf1)%laction )  CALL oas_clm_snd( jps_tsf1, isec, fsnd(:,:,jps_tsf1), info )
  IF( ssnd(jps_qsf1)%laction )  CALL oas_clm_snd( jps_qsf1, isec, fsnd(:,:,jps_qsf1), info )
!MU (12.04.13)
  IF( ssnd(jps_fpsn)%laction )  CALL oas_clm_snd( jps_fpsn, isec, fsnd(:,:,jps_fpsn), info )
  IF( ssnd(jps_fplres)%laction )  CALL oas_clm_snd( jps_fplres, isec, fsnd(:,:,jps_fplres), info )
!MU (12.04.13)
!CPS       isec = dtime * get_nstep()

  ! Performances measurement
  IF ( IOASISDEBUGLVL == 2 ) THEN
    IF ( info == PRISM_Sent     .OR. info == PRISM_ToRest .OR.   &
            & info == PRISM_SentOut  .OR. info == PRISM_ToRestOut ) &
            WRITE(6, *)  'oasclm: send_fld_2cos: before send ', toc
  ENDIF


  DEALLOCATE(fsnd)
  DEALLOCATE(snd_tmp, snd_tmp2, snd_field)

 ENDIF     !masterproc

DEALLOCATE(albt_gcell)
