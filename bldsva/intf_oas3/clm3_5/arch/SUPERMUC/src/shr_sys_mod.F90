!===============================================================================
! SVN $Id: shr_sys_mod.F90 3018 2007-01-30 22:59:40Z kauff $
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/csm_share/trunk_tags/share3_070321/shr/shr_sys_mod.F90 $
!===============================================================================
...
MODULE shr_sys_mod
   ...
   use ifport ! intel compiler portability functions
   implicit none
...
SUBROUTINE shr_sys_system(str,rcode)
   ...
   character(SHR_KIND_CL) :: cmd,file1,file2  ! command & two filenames
   integer(SHR_KIND_IN)   :: iostat           ! return code for read() command
#endif
...
!-------------------------------------------------------------------------------
   ...
   file1 = ' '
   file2 = ' '
   read(str,*,iostat=iostat) cmd
   if ( iostat /= 0 ) then
      write(6,F00) 'Catamount ERROR: ',trim(str)
      rcode = -1
   elseif ( trim(cmd) == 'rm') then
      read(str,*,iostat=iostat) cmd,file1
      if ( iostat == 0 ) then
         call unlink(trim(file1))
         write(6,F00) 'Catamount unlink: ',trim(str)
         rcode = 0
      else
         write(6,F00) 'Catamount ERROR: unlink: ',trim(str)
         rcode = -1
      end if
   elseif (trim(cmd) == 'mv') then
      read(str,*,iostat=iostat) cmd,file1,file2
      if ( iostat == 0 ) then
         call rename(trim(file1),trim(file2))
         write(6,F00) 'Catamount rename: ',trim(str)
         rcode = 0
      else
         write(6,F00) 'Catamount ERROR: rename: ',trim(str)
         rcode = -1
      endif
   else
      write(6,F00) 'Catamount ERROR: unrecognized command: ',trim(str)
      rcode = -1
   endif
#endif
