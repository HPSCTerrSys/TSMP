...
check
  if [[ $compiler == "Gnu" ]]; then
     comment "   sed comF90 based on Gnu to cos Makefile"
        ...
        sed -i "s@__comF90__@scorep-mpif90 -cpp -c -ffree-line-length-0 -fstack-protector-all -finit-integer=-2147483648 -finit-character=127@" $file >> $log_file 2>> $err_file
     else
        sed -i "s@__comF90__@$profComp $mpiPath/bin/mpif90 -cpp -c -ffree-line-length-0 -fstack-protector-all -finit-integer=-2147483648 -finit-character=127@" $file >> $log_file 2>> $err_file
     fi
     check
