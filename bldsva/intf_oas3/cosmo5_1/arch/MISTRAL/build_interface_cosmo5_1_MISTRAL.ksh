...
comment "   sed comF90 to cos Makefile"
  sed -i "s@__comF90__@$profComp $mpiPath/bin/mpif90 -cpp -c -O2 -c -fpp  -fp-model source -xCORE-AVX2@" $file >> $log_file 2>> $err_file
check
comment "   sed ld to cos Makefile"
  sed -i "s@__ld__@$profComp $mpiPath/bin/mpif90@" $file >> $log_file 2>> $err_file
check
comment "   sed libs to cos Makefile; libgrib: $gribPath"
  sed -i "s@__lib__@-L$gribPath/lib/ $cplLib -L$ncdfPath/lib/ -lgrib_api_f90 -lgrib_api -lnetcdff@" $file >> $log_file 2>> $err_file
check
route "${cblue}<< configure_cos${cnormal}"
}
