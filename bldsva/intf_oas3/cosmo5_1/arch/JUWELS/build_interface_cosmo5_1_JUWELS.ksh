...
comment "   cp Makefile to cosmo dir"
   cp $rootdir/bldsva/intf_oas3/${mList[2]}/arch/$platform/config1/Makefile $cosdir >> $log_file 2>> $err_file
check
   cp $rootdir/bldsva/intf_oas3/${mList[2]}/arch/$platform/config1/Fopts $cosdir >> $log_file 2>> $err_file
check
  c_configure_cos
...
route "${cblue}>> substitutions_cos${cnormal}"
 ...
 comment "   cp ObjFiles & ObjDependencies in $cosdir"
   patch "$rootdir/bldsva/intf_oas3/${mList[2]}/arch/$platform/config1/Obj*" $cosdir 
 check
