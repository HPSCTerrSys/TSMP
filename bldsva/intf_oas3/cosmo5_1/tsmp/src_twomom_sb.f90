!!! linked ireals to dp and imp_reals to imp_double

!!! Bugfix: corrected a wrong value in the Segal/Khain table

!!! 20140312_UB: Bug fix in clnuc_sk_4D(): clipping of cloud number to N_cn0 was done at the wrong place
!!!                                        and had nearly no effect. Now this really eliminates the
!!!                                        unrealistic high values coming from the advection scheme,
!!!                                        as was intended.

!!! Bugfix: corrected S0 in n_ice_huffmann_vali() and n_ice_huffmann_vali_vec

!!! changed wrong clipping f_v = max(f_v,1.0d0) to correct f_v = max(f_v,a_f/ice%a_ven) in all
!!!  vapor_deposition_xxx - routines

!!! old ice nucleation "ice_nucleation_vec()" re-activated by checking
!!! "nuc_i_typ < 1000" in SR clouds().

!!! D_v-Formel in ice_nucleation_homhet() mit Exp. 1.94 ist etwas niedriger (5%-10%) als
!!! die Laborwerte. Besser vielleicht die Formel mit Exp. 1.81 verwenden!

!!! ice_nucleation_vec() wird bei nuc_i_typ > 1000 verwendet,
!!! d.h. es ist komplett deaktiviert. Sollten wir da nicht besser
!!! einen eigenen Schalter einbauen, der zwischen der "alten" Methode und
!!! Lohman und Kärcher (2006) "homhet" unterscheidet?

!!! icecirrus eingefuehrt

!!! cloud_freeze:
!!! - "IF (q_c > 0.0) THEN" eingefuegt (spart Rechenaufwand)
!!! - wenn const. qnc angenommen wird, dann Limitierung des Gefrierens
!!!   (qnc) darauf, dass hoechstens die durch qnc_const
!!!   gegebene Anzahldichte nur 1 x zu Eispartikeln gefrieren kann.
!!!   Andernfalls wuerden durch die numerische Nach-Entstehung von Wolkentropfenanzahl
!!!   und nachfolgendes Gefrieren zu hohe
!!!   Eisanzahldichten entstehen.

!!! in icecry2test x_max = 1e-7 durch 1e-6 ersetzt (groessere Eispartikel erlaubt)

!!! alle REAL(KIND=8) durch DOUBLE PRECISION ersetzt (ueberall)

!!! enhanced melting nur fuer graupel und hail implementiert. Muesste es
!!! nicht insbesondere auch fuer Eis und Schnee gemacht werden (-> Nimbostratus!)?

!!! Speicherung der Umwandlungsraten: Schmelzraten von Eis und Schnee muessen
!!! aufsummiert werden (nicht einfach nur mit Werten belegen), weil diese
!!! mehrfach vorkommen:
!!! dqdt(i,j,k,29) = melt_q --> dqdt(i,j,k,29) = dqdt(i,j,k,29) + melt_q
!!! dqdt(i,j,k,30) = melt_q --> dqdt(i,j,k,30) = dqdt(i,j,k,30) + melt_q
!!! Das Feld dqdt wird standardmaessig mit 0.0 initialisiert, so dass keine weiteren
!!! Aenderungen notwendig waren.

!!! Die Hoehenkorrektur der Fallgeschw. bei den
!!! Kollisionsintegralen und der Sedimentation auf
!!! eine Formulierung mit vom mittleren Durchmesser abh. Exponenten
!!! der Dichtekorrektur verwendet, und zwar unterschiedlich fuer
!!! c, r, i, s, g und h.
!!! Damit das technisch funktioniert, wird das Verh. der Dichten
!!! moeglichst nur einmal pro Zeitschritt bestimmt und die einzelnen
!!! Prozess-Subroutinen kleben dann den Exponenten dran.
!!! Damit das bei der Sedimentation nicht doppelt gemacht wird, muss
!!! die Konstante rho_vel weiter unten auf 1.0d0 gesetzt werden!

!!! new graupel graupelhail2test4 with reduced fallspeed of small graupel,
!!! at the same time increased speed of very large graupel.
!!! Fit obtained by fallgesetz_graupel.m

!!! added new namelist parameter variables for Segal/Khain:
!!! lincloud=.true./.false.  -- allow/do not allow incloud nucleation (in src_gscp.F90.20090316)
!!! z_ncn_segal=2000.0m       -- constant N_cn below that height, exp. decrease above (hier)
!!! z1oe_ncn_segal=1000.0m    -- 1/e-scale-height of exp. decrease of N_cn above z_ncn_segal (hier)

!!! allow shifting of homog./heterogeneous cloud/rain freezing to lower temperatures
!!! by namelist-switch tshift_cloudfreeze

!!! heterogeneous cloud freezing activated

!!! cloud = cloudfastautoconv, not cloudnue1mue1 !!!
!!! (cloudstandard has the fastest autoconversion)

!!! graupel_to_snow = .true. !!!

!!! nrates (number of output conversion rates) increased by 6 (profiles of horiz. averaged mean diameter)
!!!
!!! in complete_ice_snow_riming() und complete_ice_snow_rim_vec():
!!!   at completing the snow_rain_riming in case of deposition > riming,
!!!   in the IF-statement of the ice-splintering process rime_qr has to be replaced by rime_q!
!!!

!!! moved declaration of speichere_umwandlungsraten to blahak_utils.f90.
!!! speichere_umwandlungsraten has been completely removed from this module
!!! and is now named speichere_dqdt!

!!! invented switch "graupel_to_snow" to allow for conversion of graupel to snow during deposition growth

!!! invented new simple parameterization of "graupel to snow" during deposition
!!! only in vapor_dep_simple up to now! not well testet!
...
!
!!! - in Segal/Khain cloud activation: linear extrapolation for NCN from 50 down to 0 m**-3! (only for clnuc_SK_4D)
!!!
...
! UB_20100107>> 
! rho_vel auf 1 gesetzt, weil dieser Exponent nun in den
! einzelen Prozess-Routinen auf einen vom mittleren Durchmesser
! abhaengigen Wert gesetzt wird.
!!$ UB: not used anymore!!  DOUBLE PRECISION, PARAMETER :: rho_vel    = 0.5d0        !..Exponent in Dichtekorrektur
!!$ UB: not used anymore!  DOUBLE PRECISION, PARAMETER :: rho_vel    = 1.0d0        !..Exponent in Dichtekorrektur
!!$ UB: not used anymore!!  DOUBLE PRECISION, PARAMETER :: rho_vel_r  = 0.4d0        !..Exponent in Dichtekorrektur fuer Regen
!!$ UB: not used anymore!  DOUBLE PRECISION, PARAMETER :: rho_vel_r  = 1.0d0        !..Exponent in Dichtekorrektur fuer Regen
!!$ UB: not used anymore!  DOUBLE PRECISION, PARAMETER :: rho_vel_c  = 1.0d0        !..fuer Wolkentropfen
! UB_20100107<< 

! UB_20100701>> Andere Hoehenkorrektur der Fallgeschwindigkeiten:
!               (Abgeleitet aus Cd(Re)-Daten von Kugeln mit sinnvollen
!                Modifikationen fuer rauhe Partikel).
! Korrekturvorschrift: v = v0 * (rhol0/rhol)^(a + b * min(Dquer,dknick) ) fuer r,i,s,g,h
!                      v = v0 * (etal0/eta0)^b fuer c
!               (a, b wurden fuer D berechnet, nicht Dqer, aber der
!                Einfachheit halber wird nicht die volle D-Abhaengigkeit
!                in die Kollisions- und Sedimentationsintegrale mitgenommen,
!                sondern im Sinne des Mittelwertsatzes eine
!                mittlere Korrektur vor die Integrale gezogen, die nur
!                vom mittleren Durchmesser Dquer abhaengt)
  DOUBLE PRECISION, PARAMETER :: rhovelcb = 1.0d0
  DOUBLE PRECISION, PARAMETER :: rhovelra = 0.375d0
  DOUBLE PRECISION, PARAMETER :: rhovelrb = 0.025d0
  DOUBLE PRECISION, PARAMETER :: rhovelrdknick = 5.0e-3   ! mm
  DOUBLE PRECISION, PARAMETER :: rhovelia = 0.375d0
  DOUBLE PRECISION, PARAMETER :: rhovelib = 0.01666667
  DOUBLE PRECISION, PARAMETER :: rhovelidknick = 3.0e-3   ! mm
  DOUBLE PRECISION, PARAMETER :: rhovelsa = 0.375d0
  DOUBLE PRECISION, PARAMETER :: rhovelsb = 0.0125
  DOUBLE PRECISION, PARAMETER :: rhovelsdknick = 10.0e-3   ! mm
  DOUBLE PRECISION, PARAMETER :: rhovelga =0.375d0
  DOUBLE PRECISION, PARAMETER :: rhovelgb =0.025d0
  DOUBLE PRECISION, PARAMETER :: rhovelgdknick = 5.0e-3   ! mm
  DOUBLE PRECISION, PARAMETER :: rhovelha =0.375d0
  DOUBLE PRECISION, PARAMETER :: rhovelhb =0.03125d0
  DOUBLE PRECISION, PARAMETER :: rhovelhdknick = 4.0e-3   ! mm
! UB_20100701<<
  ...
  DOUBLE PRECISION, PARAMETER :: ice_s_vel  = 0.25         !..Dispersion der Fallgeschw. 
...
! AS_2010>> Parameters for Phillips (2008) heterogeneous ice nucleation param.
!           These parameters are the default for nuc_i_typ = 4:
  DOUBLE PRECISION :: &
...
! UB_20100421>> von integer nach double geaendert:
  DOUBLE PRECISION, PARAMETER :: &
    ttstep = 2.0d0,       &  ! increment for temperature in look-up table
    ssstep = 1.0d0           ! increment for ice supersaturation in look-up table

  !.. Lookup-table fields for Karcher and Lohmann- ice nucleation
  DOUBLE PRECISION, DIMENSION(0:100,0:100), SAVE :: &
...
! AS_2010<<
  ...
  DOUBLE PRECISION, PARAMETER :: D_rainfrz_ig = 0.5d-3 !  rain --> ice oder graupel
  ...
  DOUBLE PRECISION, PARAMETER :: D_krit_ii = 50.00d-6 ! D-Schwellenwert fuer ice_selfcollection
  ...
  ! Set this switch to true if intended to use exponential DSD 
  ! for evaporation. Only effective if use_mu_Dm_rain_evap = .false. and cloud_typ > 1:
  ...
  ! ub: Schalter fuer die Umwandlung von Graupel (nicht Hagel) in Schnee
  ! beim Depositionswachstum ( bei vapor_deposition_growth() ganz einfache Param.: Rate = 3 * Dep.rate;
  ! bei vapor_dep_simple() neue Parametrisierung unter Ber. der Riming-Rate von Graupel)
  LOGICAL, PARAMETER           :: graupel_to_snow = .true.
  !.. Tuning faktoren fuer graupel to snow Umwandlung:
  !.. Proportionalitaetsfaktor zwischen Umwandlungsrate und inverser integraler Prozess-Zeitskala:
  DOUBLE PRECISION, PARAMETER  :: gr_sn_conv_tune_1 = 1.5 ! wirkt bei Erhoehung verstaerkend
  !.. Faktor, um den die depos. mind. groesser sein muss als das riming, damit 
  !   die graupel to snow Umwandlung ueberhaupt einsetzt:
  DOUBLE PRECISION, PARAMETER  :: gr_sn_conv_tune_2 = 2.0 ! wirkt bei Erhoehung abschwaechend
  DOUBLE PRECISION, PARAMETER  :: D_conv_gs = 500.0d-6    ! Max. D_s fuer Schnee erzeugt aus graupel durch gr.dep.
  DOUBLE PRECISION, PARAMETER  :: D_g_max_gs = 2000.0d-6  ! Max. D_g fuer "graupel_to_snow"
...
! <<hn
  ...
  TYPE(PARTICLE), PARAMETER :: graupelhail2test = PARTICLE( & ! 'graupel/hail Uli2'
       &                                 'graupelhail2test' ,& !.name...Bezeichnung
       &                                 1.000000, & !.nu.....Breiteparameter der Verteil.
       &                                 0.333333, & !.mu.....Exp.-parameter der Verteil.
  ...
  TYPE(PARTICLE), PARAMETER :: graupelhail2test2 = PARTICLE( & ! 'graupel/hail Uli2_2'
       &                                 'graupelhail2test2' ,& !.name...Bezeichnung
       &                                 1.000000, & !.nu.....Breiteparameter der Verteil.
       &                                 0.333333, & !.mu.....Exp.-parameter der Verteil.
!       &                                 1.00d-03, & !.x_max..maximale Teilchenmasse
  ...
  TYPE(PARTICLE), PARAMETER :: graupelhail2test3 = PARTICLE( & ! 'graupel/hail Uli2_3'
       &                                 'graupelhail2test3' ,& !.name...Bezeichnung
       &                                 1.000000, & !.nu.....Breiteparameter der Verteil.
       &                                 0.333333, & !.mu.....Exp.-parameter der Verteil.
!       &                                 1.00d-03, & !.x_max..maximale Teilchenmasse
       &                                 5.00d-04, & !.x_max..maximale Teilchenmasse
  ...
  TYPE(PARTICLE), PARAMETER :: graupelhail2test4 = PARTICLE( & ! 'graupel/hail Uli2_4'
       &                                 'graupelhail2test4' ,& !.name...Bezeichnung
       &                                 1.000000, & !.nu.....Breiteparameter der Verteil.
       &                                 0.333333, & !.mu.....Exp.-parameter der Verteil.
!       &                                 1.00d-03, & !.x_max..maximale Teilchenmasse
       &                                 5.00d-04, & !.x_max..maximale Teilchenmasse
...
!       &                                 4.00d-03, & !.x_max..maximale Teilchenmasse
  ...
  TYPE(PARTICLE), PARAMETER :: iceCRY2test =  PARTICLE( & ! nach Andy Heymsfield (CRYSTAL-FACE)
       ...
       &                              0.333333, & !.mu.....Exp.-parameter der Verteil.
! AS_2010:       &                              1.00d-07, & !.x_max..maximale Teilchenmasse D=???e-2m
...
! AS_2010 <<
  ...
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION (:,:,:) :: rrho_04
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION (:,:,:) :: rrho_c

  DOUBLE PRECISION :: rain_cmu0,rain_cmu1,rain_cmu2,rain_cmu3,rain_cmu4,rain_gfak
  INTEGER          :: rain_cmu5

  !====================================================
  ! Variables for namelist parameters:
  !====================================================

  DOUBLE PRECISION :: qnc_const
  DOUBLE PRECISION :: tshift_cloudfreeze
  DOUBLE PRECISION :: z_ncn_segal
  DOUBLE PRECISION :: z1oe_ncn_segal
...
! UB_20100701>>
  DOUBLE PRECISION FUNCTION dyn_visc_air(T)
    !*******************************************************************************
    !                                                                              *
    !       Dynamic viscosity eta of dry air as function of temperature T          *
    !       (linear Fit to the Sutherland-formula by myself)                        *
    !                                                                              *
    !       Valid for T in [203.16; 313.16] K and 0.1 bar < p < 10 bar             *
    !                                                                              *
    !       Input T is in Kelvin                                                   *
    !       Output eta is in Pa s                                                  *
    !                                                                              *
    !*******************************************************************************
    ...
    dyn_visc_air = 2.92332d-6 + 5.20712d-8 * T
...
!  LOGICAL          :: speichere_umwandlungsraten = .FALSE.
  ...
  ! ... Gitter ...
  INTEGER          :: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz

#ifdef COSMOART
   INTEGER, DIMENSION(:), ALLOCATABLE :: calc_i,calc_j,calc_k
  ...
  ! Anzahl der Umwandlungsraten + Anzahl der Sedifluxdivergenzen + 24
  ! (1 fuer die Heizrate, 1 fuer Temperaturprofil, 4 fuer Uebersaett. vor/nach Mikrophysik , 
  !  6 fuer die Q's, 6 fuer die QN's, 6 fuer die DM's); 
  ! Die Umwandlungsraten beginnen bei 1 und gehen bis nrates-18;
  ! nrates-23 und nrates-22 ist fuer die Uebersaettigung geg. Wasser vor und nach der Mikrophysik;
  ! nrates-21 und nrates-20 ist fuer die Uebersaettigung geg. Eis vor und nach der Mikrophysik;
  ! nrates-19 und nrates-18 ist fuer die Heizrate und fuers T-Profil;
  ! nrates-17 bis nrates-12 ist fuer QC,QR,QI,QS,QG und QH;
  ! nrates-11 bis nrates-6 ist fuer QNC,QNR,QNI,QNS,QNG und QNH
  ! nrates-5 bis nrates ist fuer DMC,DMR,DMI,DMS,DMG und DMH
  INTEGER, PARAMETER :: nmicrorates = 63
  INTEGER, PARAMETER :: nsedifluxdiv = 12
  INTEGER, PARAMETER :: nrates = nmicrorates + nsedifluxdiv + 24
...
CONTAINS
  ...
  SUBROUTINE alloc_driver ()
    ...
    IF (firstcall /= 1) THEN
      ...
      ALLOCATE(     w(0:dim_ix,1:dim_iy,1:dim_iz), &
           &        p(0:dim_ix,1:dim_iy,1:dim_iz), &
           &        T(0:dim_ix,1:dim_iy,1:dim_iz), &
           &      rho(0:dim_ix,1:dim_iy,1:dim_iz), &
           &        q(0:dim_ix,1:dim_iy,1:dim_iz), &
           &      S_w(0:dim_ix,1:dim_iy,1:dim_iz), &
           &      S_i(0:dim_ix,1:dim_iy,1:dim_iz), &
           &    dSwdz(0:dim_ix,1:dim_iy,1:dim_iz), &
           &    dSidz(0:dim_ix,1:dim_iy,1:dim_iz), &
           &    dT0dz(0:dim_ix,1:dim_iy,1:dim_iz), &
           &    STAT = stat_var )


#ifdef COSMOART
      ALLOCATE( cloudact(0:dim_ix,1:dim_iy,1:dim_iz), STAT = stat_var)        
      IF (stat_var /= 0) THEN
        WRITE (*,*) 'Fehler bei Allokierung cloudact', stat_var
        STOP
      END IF
#endif 
      ...
      ALLOCATE( zml_k(0:dim_ix,1:dim_iy,1:dim_iz), STAT = stat_var)        
      IF (stat_var /= 0) THEN
        WRITE (*,*) 'Fehler bei Allokierung zml_k ', stat_var
        STOP
      END IF
      ALLOCATE( w_cb(0:dim_ix,1:dim_iy,1:dim_iz), STAT = stat_var)        
      IF (stat_var /= 0) THEN
      ...
      END IF
       ...
       IF (use_ice_graupel_conv_uli) THEN
        ALLOCATE( deprate_ice(0:dim_ix,1:dim_iy,1:dim_iz), &
             deprate_snow(0:dim_ix,1:dim_iy,1:dim_iz), &
             STAT = stat_var)        
        IF (stat_var /= 0) THEN
          WRITE (*,*) 'Fehler bei Allokierung deprate_ice, deprate_snow ', stat_var
          STOP
        END IF
        ALLOCATE( rimeqcrate_ice(0:dim_ix,1:dim_iy,1:dim_iz), &
             rimencrate_ice(0:dim_ix,1:dim_iy,1:dim_iz), &
             rimeqirate_ice(0:dim_ix,1:dim_iy,1:dim_iz), &
             rimeqrrate_ice(0:dim_ix,1:dim_iy,1:dim_iz), &
             rimenrrate_ice(0:dim_ix,1:dim_iy,1:dim_iz), &
             rimeqcrate_snow(0:dim_ix,1:dim_iy,1:dim_iz), &
             rimencrate_snow(0:dim_ix,1:dim_iy,1:dim_iz), &
             rimeqirate_snow(0:dim_ix,1:dim_iy,1:dim_iz), &
             rimeqrrate_snow(0:dim_ix,1:dim_iy,1:dim_iz), &
             rimenrrate_snow(0:dim_ix,1:dim_iy,1:dim_iz), &
             d_id_sp(0:dim_ix,1:dim_iy,1:dim_iz), &
             d_sd_sp(0:dim_ix,1:dim_iy,1:dim_iz), &
             d_rd_sp_ice(0:dim_ix,1:dim_iy,1:dim_iz), &
             d_rd_sp_snow(0:dim_ix,1:dim_iy,1:dim_iz), &
             STAT = stat_var)        
        IF (stat_var /= 0) THEN
...
Module globale_variablen
  USE wolken_driver, ONLY: dt, loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, &
       & w, T, p, q, rho,       &
       & q_cloud, n_cloud,    &
       & q_rain, n_rain,       &
       & q_ice, n_ice,         &
       ...
       & q_graupel, n_graupel,   &
       & q_hail, n_hail,   &
       & S_w, dSwdz, S_i, dSidz, dT0dz, zml_k, w_cb, &
       & dqdt, speichere_dqdt, nrates, nmicrorates, &
...
#ifdef COSMOART
  USE wolken_driver, ONLY:cloudact,calc_i,calc_j,calc_k
#endif
! ub<<      
END Module globale_variablen

Module konstanten
  USE wolken_driver, ONLY: wolke_typ, pi, r, r1, cv, cp
...
CONTAINS
  ...
  DOUBLE PRECISION FUNCTION global_maxval(feld)
    ...
    USE data_parameters , ONLY :     &
         ...
         & ireals => dp                     ! KIND-type parameters for real variables
    USE data_parallel , ONLY:        &
         & icomm_cart,               & ! communicator for the virtual cartesian topology
         & imp_reals => imp_double     ! determines the correct REAL type used in the model
    IMPLICIT NONE
...
CONTAINS
  ...
  SUBROUTINE sedi_lm_loop_vec (nx, qx, v_n_sedi, v_q_sedi, adz, dt, q_flux, &
    ...
    INTEGER(KIND=iintegers), INTENT(IN) :: ims,ime,jms,jme,kms,kme
    REAL(KIND=ireals), DIMENSION( ims:ime , jms:jme , kms:kme ),    &
         INTENT(inout) :: nx, qx
    REAL(KIND=ireals), DIMENSION( ims:ime , jms:jme , kms:kme ),    &
         INTENT(in)    :: adz
    REAL(KIND=ireals), INTENT(in) :: dt
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms-1:kme), &
         INTENT(inout) :: q_flux
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms:kme+1), &
         INTENT(inout) :: v_n_sedi,v_q_sedi

    !=======================================================
    ! .. Local variables:

    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms-1:kme) :: n_flux
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms:kme)   :: dz
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme) :: &
         v_nv, v_qv, s_nv, s_qv, c_nv, c_qv
    REAL(KIND=ireals) :: odt

    INTEGER(KIND=iintegers)  :: i, j, k, kk
    

    !=======================================================
    ! .. Vertical grid distance dz:
!CDIR COLLAPSE
    dz = 1.0_ireals / adz

    !=======================================================
    ! .. Inverse sedimentation time step:
    odt = 1.0_ireals / dt

    !=======================================================
    ! .. lower and upper boundary conditions for the calculation of
    !    the fluxes and flux divergences:

!!$ UB: not needed if below v_x = v_x_sedi(i,j,k)!!
!CDIR ON_ADB(v_n_sedi,v_q_sedi)
    v_n_sedi(:,:,kme+1) = v_n_sedi(:,:,kme)   !
    v_q_sedi(:,:,kme+1) = v_q_sedi(:,:,kme)   ! lower BC for the terminal velocities

    n_flux(:,:,kms-1) = 0.0_ireals ! 
    q_flux(:,:,kms-1) = 0.0_ireals ! upper BC on fluxes


    !=======================================================
    ! .. The sedimentation loop in vectorized version:

    DO k = kms,kme

!CDIR ON_ADB(v_n_sedi,v_q_sedi,v_nv,v_qv,c_nv,c_qv)
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
!!$ UB: not so good, because this is the downward flanking speed, not
!!$     representative of the flux coming from above:
!!$          v_nv(i,j) = 0.5_ireals * (v_n_sedi(i,j,k+1)+v_n_sedi(i,j,k))
!!$          v_qv(i,j) = 0.5_ireals * (v_q_sedi(i,j,k+1)+v_q_sedi(i,j,k))
!!$ UB: probably better in the sense of a low-order explicit scheme:
          v_nv(i,j) = v_n_sedi(i,j,k)
          v_qv(i,j) = v_q_sedi(i,j,k)

          ! Formulierung unter der Annahme, dass v_nv, v_qv stets negativ
          c_nv(i,j) = -v_nv(i,j) * adz(i,j,k) * dt 
          c_qv(i,j) = -v_qv(i,j) * adz(i,j,k) * dt
        END DO
      END DO

      kk = k
!CDIR ON_ADB(adz,dz,nx,s_nv,c_nv)
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          s_nv(i,j) = nx(i,j,k) * dz(i,j,k) * MIN(c_nv(i,j),1.0_ireals)
        END DO
      END DO
      DO
        IF (kk <= kms) EXIT
!CDIR COLLAPSE
        IF (MAXVAL(c_nv) <= 1.0_ireals) EXIT
        kk  = kk - 1
!CDIR ON_ADB(adz,dz,nx,s_nv,c_nv)
!CDIR COLLAPSE
        DO j = jms,jme
          DO i = ims,ime
            IF (c_nv(i,j) > 1.0_ireals) THEN
              c_nv(i,j) = (c_nv(i,j) - 1.0_ireals) * adz(i,j,kk) * dz(i,j,kk+1)
              s_nv(i,j) = s_nv(i,j) + nx(i,j,kk) * dz(i,j,kk) * MIN(c_nv(i,j),1.0_ireals)
            END IF
          END DO
        END DO
      ENDDO
      ...
      kk = k
!CDIR ON_ADB(adz,dz,qx,s_qv,s_nv,c_qv)
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          s_qv(i,j) = qx(i,j,k) * dz(i,j,k) * MIN(c_qv(i,j),1.0_ireals)
        END DO
      END DO
      DO 
        IF (kk <= kms) EXIT
!CDIR COLLAPSE
        IF (MAXVAL(c_qv) <= 1.0_ireals) EXIT
        kk  = kk - 1
!CDIR ON_ADB(adz,dz,qx,s_qv,s_nv,c_qv)
!CDIR COLLAPSE
        DO j = jms,jme
          DO i = ims,ime
            IF (c_qv(i,j) > 1.0_ireals) THEN
              c_qv(i,j) = (c_qv(i,j) - 1.0_ireals) * adz(i,j,kk) * dz(i,j,kk+1)
              s_qv(i,j) = s_qv(i,j) + qx(i,j,kk) * dz(i,j,kk) * MIN(c_qv(i,j),1.0_ireals)
            END IF
          END DO
        END DO
      ENDDO
...
!CDIR ON_ADB(dz,qx,nx,s_qv,s_nv)
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          s_nv(i,j) = -s_nv(i,j) * odt
          s_qv(i,j) = -s_qv(i,j) * odt
          n_flux(i,j,k) = MAX(s_nv(i,j),n_flux(i,j,k-1)-nx(i,j,k) * dz(i,j,k)*odt)
          q_flux(i,j,k) = MAX(s_qv(i,j),q_flux(i,j,k-1)-qx(i,j,k) * dz(i,j,k)*odt)
        END DO
      END DO
...
!! !CDIR ON_ADB(adz,n_flux,q_flux)
    DO k = kms,kme
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          nx(i,j,k) = nx(i,j,k) + ( n_flux(i,j,k) - n_flux(i,j,k-1) )*adz(i,j,k)*dt
          qx(i,j,k) = qx(i,j,k) + ( q_flux(i,j,k) - q_flux(i,j,k-1) )*adz(i,j,k)*dt
        ENDDO
      ENDDO
    ENDDO


  END SUBROUTINE sedi_lm_loop_vec

  !================================================================================
  !
  !  Sedimentation loop: Updates to the fields nx and qx due to the divergence
  !  of the sedimentation flux, where the sedimentation velocities are given
  !  in the input fields  v_n_sedi and v_q_sedi, resp.
  !
  !  The fields nx, qx and v_n_sedi, v_q_sedi are defined on the center 
  !  points of the grid cells. The fluxes itself are computed using
  !  ideas about integer and fractional courant number from 
  !  Lin and Rood (1996) using a first order 1-dim. Scheme, with fluxes
  !  defined at the grid cell faces. For this, sedimentation velocities
  !  are first linearily interpolated to the grid cell boundaries.
  !
  !  NOTE: the k-index increases from 1 at the top to kme at the bottom!
  !
  !================================================================================

  SUBROUTINE sedi_lm_loop (nx, qx, v_n_sedi, v_q_sedi, adz, dt, q_flux, &
       &             ims,ime,jms,jme,kms,kme)
    ...
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms-1:kme) :: n_flux
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms:kme)   :: dz
    REAL(KIND=ireals) :: v_n, v_q, s_n, s_q, c_n, c_q
    REAL(KIND=ireals) :: odt
...
!!$ UB: not so good, because this is the downward flanking speed, not
!!$     representative of the flux coming from above:
!!$          v_n = 0.5_ireals * (v_n_sedi(i,j,k+1)+v_n_sedi(i,j,k))
!!$          v_q = 0.5_ireals * (v_q_sedi(i,j,k+1)+v_q_sedi(i,j,k))
!!$ UB: probably better in the sense of a low-order explicit scheme:
          v_n = v_n_sedi(i,j,k)
          v_q = v_q_sedi(i,j,k)
          ...
          kk = k
          s_n = nx(i,j,kk) * dz(i,j,kk) * MIN(c_n,1.0_ireals);
          DO WHILE (c_n > 1.0_ireals .AND. kk > kms)
            kk = kk - 1
            c_n = (c_n - 1.0_ireals) * adz(i,j,kk) * dz(i ,j,kk+1)
            s_n = s_n + nx(i,j,kk) * dz(i,j,kk) * MIN(c_n,1.0_ireals)
          END DO    
          s_n = -s_n * odt;

          kk = k
          s_q = qx(i,j,kk) * dz(i,j,kk) * MIN(c_q,1.0_ireals);
          DO WHILE (c_q > 1.0_ireals .AND. kk > kms)
            kk = kk - 1
            c_q = (c_q - 1.0_ireals) * adz(i,j,kk) * dz(i ,j,kk+1)
            s_q = s_q + qx(i,j,kk) * dz(i,j,kk) * MIN(c_q,1.0_ireals)
          END DO    
          s_q = -s_q * odt;

          ! Flux-limiter to avoid negative values
          n_flux(i,j,k) = MAX(s_n,n_flux(i,j,k-1)-nx(i,j,k) * dz(i,j,k)*odt)
          q_flux(i,j,k) = MAX(s_q,q_flux(i,j,k-1)-qx(i,j,k) * dz(i,j,k)*odt)
  ...
  END SUBROUTINE sedi_lm_loop

  SUBROUTINE rain_sedimentation_lm (qr,nr,qc,rainrate,rho,rhocorr,adz,dt, &
       &             ims,ime,jms,jme,kms,kme ) ! dims of input fields
    !*******************************************************************************
    ...
    ! .. Local Variables ..
    INTEGER(KIND=iintegers)        :: i, j, k, kk
    INTEGER(KIND=iintegers), SAVE  :: firstcall
    REAL(KIND=ireals)       :: n_r,q_r,x_r,n0,lam,G1,G4,D_m,D_r,mue,v_n,v_q,rhoexp

    REAL(KIND=ireals), PARAMETER :: alf = 9.65e+00_ireals     ! in SI [m/s]
    REAL(KIND=ireals), PARAMETER :: bet = 1.03e+01_ireals     ! in SI [m/s]
    REAL(KIND=ireals), PARAMETER :: gam = 6.00e+02_ireals     ! in SI [1/m]
    REAL(KIND=ireals), PARAMETER :: n_0 = 1.00e+07_ireals     ! in SI [1/m^4]
    REAL(KIND=ireals), PARAMETER :: eps = 1.00e-20_ireals  

!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms-1:kme)   :: q_fluss
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms:kme+1)   :: v_n_rain,v_q_rain
    ...
    IF (firstcall.NE.1) THEN
      IF(isdebug) THEN
#ifdef SEDI_VECTORIZED
        WRITE (*,*)           "CLOUD rain_sedimentation_lm vectorized:"
#else
        WRITE (*,*)           "CLOUD rain_sedimentation_lm:"
#endif
      ENDIF
      firstcall = 1
    ENDIF

    WHERE (nr < 0.0_ireals) nr = 0.0_ireals
    WHERE (qr < 0.0_ireals) qr = 0.0_ireals
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          ...
          qr(i,j,k) = rho(i,j,k)*qr(i,j,k)
          nr(i,j,k) = rho(i,j,k)*nr(i,j,k)
          n_r = nr(i,j,k) + eps        !..Anzahldichte in SI
          q_r = qr(i,j,k)              !..Fluessigwassergehalt in SI
          IF (cloud_typ <= 1) THEN
          ...
          ELSEIF (use_mu_Dm_rain_sedi) THEN
            ...
            D_m = ( 6.0_ireals / (rho_w*pi) * x_r )**(1.0_ireals/3.0_ireals) 
            IF (qc(i,j,k) >= eps) then ! Seifert (2007)            
              ! UB_20080212>
              !              mue = 2.0_ireals
              mue = (rain%nu+1.0_ireals)/rain%b_geo - 1.0_ireals
              ! <UB_20080212
            ELSEIF (D_m.le.rain_cmu3) then    
              mue = rain_cmu0*TANH((4.0_ireals*rain_cmu2*(D_m-rain_cmu3))**2) &
                   & + rain_cmu4
            ELSE
              mue = rain_cmu1*TANH((1.0_ireals*rain_cmu2*(D_m-rain_cmu3))**2) &
                   & + rain_cmu4
            ENDIF
            D_r = (D_m**3/((mue+3.0_ireals)*(mue+2.0_ireals)*(mue+1.0_ireals)))**(1.0_ireals/3.0_ireals)
          ...
          ELSE
            ...
            ELSE
              x_r = q_r / n_r                           !..mittlere Masse in SI
              x_r = MIN(MAX(x_r,rain%x_min),rain%x_max)
              n0  = n_r * (G4/x_r)**(1.0_ireals/3.0_ireals)             !..n0
              n0  = MIN(MAX(250.0e3_ireals,n0),20000.0e3_ireals)
              lam = (G4*n0/(q_r+eps))**(0.25_ireals)                 !..lambda
            ...
            ! UB_20090316<<
          ENDIF
          v_n = alf - bet / (1.0_ireals + gam*D_r)**(mue+1.0_ireals)
          v_q = alf - bet / (1.0_ireals + gam*D_r)**(mue+4.0_ireals)
          ! UB_20100107>>
          rhoexp = rhocorr(i,j,k)**(rhovelra+rhovelrb*MIN(D_r,rhovelrdknick))
          v_n = v_n * rhoexp
          v_q = v_q * rhoexp
          ! UB_20100107<<
          v_n = MAX(v_n,0.1_ireals)
          v_q = MAX(v_q,0.1_ireals)
          v_n = MIN(v_n,20.0_ireals)
          v_q = MIN(v_q,20.0_ireals)
          v_n_rain(i,j,k) = - v_n
          v_q_rain(i,j,k) = - v_q
    ...
    !===========================================================
    ! .. Calculate sedimentation fluxes and update nr, qr due to
    !    flux divergences:

#ifdef SEDI_VECTORIZED
    CALL sedi_lm_loop_vec (nr, qr, v_n_rain, v_q_rain, adz, dt, q_fluss, &
         &             ims,ime,jms,jme,kms,kme)
#else
    CALL sedi_lm_loop     (nr, qr, v_n_rain, v_q_rain, adz, dt, q_fluss, &
         &             ims,ime,jms,jme,kms,kme)
#endif

    !===========================================================
    ! .. Convert nr, qr back to mass specific quantities and
    !    store precipitation flux at the surface:
...
!CDIR COLLAPSE
  ...
  SUBROUTINE graupel_sedimentation_lm (qg,ng,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER(KIND=iintegers)         :: i, j, k, kk
    INTEGER(KIND=iintegers), SAVE   :: firstcall
    REAL(KIND=ireals)     :: n_g,q_g,x_g,lam,n,v_n,v_q,D_g,rhoexp
    REAL(KIND=ireals), SAVE      :: alf_n,alf_q,c_lam
    REAL(KIND=ireals), PARAMETER :: eps = 1.00e-20_ireals

!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms-1:kme)   :: q_fluss
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms:kme+1)   :: v_n_sedi,v_q_sedi

    !==================================================
    ! .. Preparations:

    IF (firstcall.NE.1) THEN
      alf_n = graupel%a_vel * gfct((graupel%nu+graupel%b_vel+1.0_ireals)/graupel%mu)&
           &                / gfct((graupel%nu+1.0_ireals)/graupel%mu)
      alf_q = graupel%a_vel * gfct((graupel%nu+graupel%b_vel+2.0_ireals)/graupel%mu)&
           &                / gfct((graupel%nu+2.0_ireals)/graupel%mu)
      c_lam = gfct((graupel%nu+1.0_ireals)/graupel%mu)/gfct((graupel%nu+2.0_ireals)/graupel%mu)
      IF(isdebug) THEN
#ifdef SEDI_VECTORIZED
        WRITE (*,*)           "CLOUD graupel_sedimentation_lm vectorized:"
#else
        WRITE (*,*)           "CLOUD graupel_sedimentation_lm:"
#endif
        WRITE (*,'(A,ES10.3)') "  Coeff. lambda:          c_lam = ",c_lam
        WRITE (*,'(A,ES10.3)') "  Coeff. sedimentation n: alf_n = ",alf_n
        WRITE (*,'(A,ES10.3)') "  Coeff. sedimentation q: alf_q = ",alf_q
      ENDIF
      firstcall = 1
    ENDIF

    WHERE (qg < 0.0_ireals) qg = 0.0_ireals
    WHERE (ng < 0.0_ireals) ng = 0.0_ireals
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          qg(i,j,k) = rho(i,j,k)*qg(i,j,k)
          ng(i,j,k) = rho(i,j,k)*ng(i,j,k)
          x_g = qg(i,j,k) / (ng(i,j,k)+eps)
          x_g = MIN(MAX(x_g,graupel%x_min),graupel%x_max)
          D_g = graupel%a_geo * x_g**graupel%b_geo
          rhoexp = rhocorr(i,j,k)**(rhovelga+rhovelgb*MIN(D_g,rhovelgdknick))
          lam = ( c_lam * x_g )**(graupel%b_vel) * rhoexp
          v_n = alf_n * lam
          v_q = alf_q * lam
          v_n = MAX(v_n,0.1_ireals)
          v_q = MAX(v_q,0.1_ireals)
          v_n = MIN(v_n,30.0_ireals)
          v_q = MIN(v_q,30.0_ireals)
          v_n_sedi(i,j,k) = -v_n
          v_q_sedi(i,j,k) = -v_q
        END DO
      END DO
    ...
    !===========================================================
    ! .. Calculate sedimentation fluxes and update ng, qg due to
    !    flux divergences:

#ifdef SEDI_VECTORIZED
    CALL sedi_lm_loop_vec (ng, qg, v_n_sedi, v_q_sedi, adz, dt, q_fluss, &
       &             ims,ime,jms,jme,kms,kme)
#else
    CALL sedi_lm_loop     (ng, qg, v_n_sedi, v_q_sedi, adz, dt, q_fluss, &
       &             ims,ime,jms,jme,kms,kme)
#endif

    !===========================================================
    ! .. Convert ng, qg back to mass specific quantities and
    !    store precipitation flux at the surface:
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          qg(i,j,k) = qg(i,j,k)  / rho(i,j,k)
          ng(i,j,k) = ng(i,j,k)  / rho(i,j,k)
          rainrate(i,j,k) = - q_fluss(i,j,k) ! graupel rate
        ENDDO
      ENDDO
    ENDDO
  ...
  SUBROUTINE hail_sedimentation_lm (qh,nh,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER(KIND=iintegers)         :: i, j, k, kk
    INTEGER(KIND=iintegers), SAVE   :: firstcall
    REAL(KIND=ireals)     :: n_g,q_g,x_g,lam,v_n,v_q,rhoexp,D_g
    REAL(KIND=ireals), SAVE      :: alf_n,alf_q,c_lam
    REAL(KIND=ireals), PARAMETER :: eps = 1.00e-20   

    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms-1:kme)   :: q_fluss
    REAL(KIND=ireals), DIMENSION(ims:ime,jms:jme,kms:kme+1)   :: v_n_sedi,v_q_sedi,zr

    !==================================================
    ! .. Preparations:

    IF (firstcall.NE.1) THEN
      alf_n = hail%a_vel * gfct((hail%nu+hail%b_vel+1.0_ireals)/hail%mu)&
           &                / gfct((hail%nu+1.0_ireals)/hail%mu)
      alf_q = hail%a_vel * gfct((hail%nu+hail%b_vel+2.0_ireals)/hail%mu)&
           &                / gfct((hail%nu+2.0_ireals)/hail%mu)
      c_lam = gfct((hail%nu+1.0_ireals)/hail%mu)/gfct((hail%nu+2.0_ireals)/hail%mu)
      IF (isdebug) THEN
#ifdef SEDI_VECTORIZED
        WRITE (*,*)           "CLOUD hail_sedimentation_lm vectorized:"
#else
        WRITE (*,*)           "CLOUD hail_sedimentation_lm:"
#endif
        WRITE (*,'(A,ES10.3)') "  Coeff. lambda:          c_lam = ",c_lam
        WRITE (*,'(A,ES10.3)') "  Coeff. sedimentation n: alf_n = ",alf_n
        WRITE (*,'(A,ES10.3)') "  Coeff. sedimentation q: alf_q = ",alf_q
    ...
    WHERE (qh < 0.0_ireals) qh = 0.0_ireals
    WHERE (nh < 0.0_ireals) nh = 0.0_ireals
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          qh(i,j,k) = rho(i,j,k)*qh(i,j,k)
          nh(i,j,k) = rho(i,j,k)*nh(i,j,k)
          x_g = qh(i,j,k) / (nh(i,j,k)+eps)
          x_g = MIN(MAX(x_g,hail%x_min),hail%x_max)
! UB_20100107>>
          D_g = hail%a_geo * x_g**hail%b_geo
          rhoexp = rhocorr(i,j,k)**(rhovelha+rhovelhb*MIN(D_g,rhovelhdknick))
! UB_20100107<<
          lam = ( c_lam * x_g )**(hail%b_vel) * rhoexp
          v_n = alf_n * lam
          v_q = alf_q * lam
          v_n = MAX(v_n,0.1_ireals)
          v_q = MAX(v_q,0.1_ireals)
    ...
    !===========================================================
    ! .. Calculate sedimentation fluxes and update nh, qh due to
    !    flux divergences:

#ifdef SEDI_VECTORIZED
    CALL sedi_lm_loop_vec (nh, qh, v_n_sedi, v_q_sedi, adz, dt, q_fluss, &
       &             ims,ime,jms,jme,kms,kme)
#else
    CALL sedi_lm_loop     (nh, qh, v_n_sedi, v_q_sedi, adz, dt, q_fluss, &
       &             ims,ime,jms,jme,kms,kme)
#endif

    !===========================================================
    ! .. Convert nh, qh back to mass specific quantities and
    !    store precipitation flux at the surface:
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          ...
          nh(i,j,k) = nh(i,j,k) / rho(i,j,k)
          rainrate(i,j,k) = - q_fluss(i,j,k) ! Hail rate
        ENDDO
      ENDDO
    ENDDO
  ...
  SUBROUTINE snow_sedimentation_lm (qs,ns,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER(KIND=iintegers)         :: i, j, k, kk
    INTEGER(KIND=iintegers), SAVE   :: firstcall
    REAL(KIND=ireals)     :: n_s,q_s,x_s,lam,v_n,v_q,mue,nue,D_m,rhoexp
    REAL(KIND=ireals), SAVE      :: alf_n,alf_q,c_lam
    REAL(KIND=ireals), PARAMETER :: eps = 1.00e-20_ireals   
...
!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    ...
    IF (firstcall.NE.1) THEN
      alf_n = snow%a_vel * gfct((snow%nu+snow%b_vel+1.0_ireals)/snow%mu) / &
           gfct((snow%nu+1.0_ireals)/snow%mu)
      alf_q = snow%a_vel * gfct((snow%nu+snow%b_vel+2.0_ireals)/snow%mu) / &
           gfct((snow%nu+2.0_ireals)/snow%mu)
      c_lam = gfct((snow%nu+1.0_ireals)/snow%mu)/gfct((snow%nu+2.0_ireals)/snow%mu)
      IF(isdebug) THEN
#ifdef SEDI_VECTORIZED
        WRITE (*,*)           "CLOUD snow_sedimentation_lm vectorized:"
#else
        WRITE (*,*)           "CLOUD snow_sedimentation_lm:"
#endif
        WRITE (*,'(A,ES10.3)') "  Coeff. lambda:          c_lam = ",c_lam
        WRITE (*,'(A,ES10.3)') "  Coeff. sedimentation n: alf_n = ",alf_n
        WRITE (*,'(A,ES10.3)') "  Coeff. sedimentation q: alf_q = ",alf_q
    ...
    WHERE (qs < 0.0_ireals) qs = 0.0_ireals
    WHERE (ns < 0.0_ireals) ns = 0.0_ireals
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          qs(i,j,k) = rho(i,j,k)*qs(i,j,k)
          ns(i,j,k) = rho(i,j,k)*ns(i,j,k)
          x_s = qs(i,j,k) / (ns(i,j,k)+eps)
          ...
          IF (use_mu_Dm_snow_sedi) THEN
            ...
            nue = (mue-2.0_ireals)/3.0_ireals
            alf_n = snow%a_vel * gfct((nue+snow%b_vel+1.0_ireals)/snow%mu)/gfct((nue+1.0_ireals)/snow%mu)
            alf_q = snow%a_vel * gfct((nue+snow%b_vel+2.0_ireals)/snow%mu)/gfct((nue+2.0_ireals)/snow%mu)
            c_lam = gfct((nue+1.0)/snow%mu)/gfct((nue+2.0_ireals)/snow%mu)
          ENDIF
! UB_20100107>>
          rhoexp = rhocorr(i,j,k)**(rhovelsa+rhovelsb*MIN(D_m,rhovelsdknick))
! UB_20100107<<
          lam = ( c_lam * x_s )**(snow%b_vel) * rhoexp
          v_n = alf_n * lam
          v_q = alf_q * lam
          v_n = MAX(v_n,0.1_ireals)
          v_q = MAX(v_q,0.1_ireals)
          v_n = MIN(v_n,3.0_ireals)
          v_q = MIN(v_q,3.0_ireals)
          v_n_sedi(i,j,k) = -v_n
          v_q_sedi(i,j,k) = -v_q
        END DO
      END DO
...
#ifdef SEDI_VECTORIZED
    CALL sedi_lm_loop_vec (ns, qs, v_n_sedi, v_q_sedi, adz, dt, q_fluss, &
       &             ims,ime,jms,jme,kms,kme)
#else
    CALL sedi_lm_loop     (ns, qs, v_n_sedi, v_q_sedi, adz, dt, q_fluss, &
       &             ims,ime,jms,jme,kms,kme)
#endif
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          ...
          ns(i,j,k) = ns(i,j,k) / rho(i,j,k)
          rainrate(i,j,k) = - q_fluss(i,j,k) ! Snow rate
        ENDDO
      ENDDO
    ENDDO
  ...
  SUBROUTINE ice_sedimentation_lm (qi,ni,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER(KIND=iintegers)         :: i, j, k, kk
    INTEGER(KIND=iintegers), SAVE   :: firstcall
    REAL(KIND=ireals)     :: n_i,q_i,x_i,lam,v_n,v_q,n,D_i,rhoexp
    REAL(KIND=ireals), SAVE      :: alf_n,alf_q,c_lam
    REAL(KIND=ireals), PARAMETER :: eps = 1.00e-20_ireals  
...
!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    ...
    IF (firstcall.NE.1) THEN
      alf_n = ice%a_vel * gfct((ice%nu+ice%b_vel+1.0_ireals)/ice%mu) / &
           gfct((ice%nu+1.0_ireals)/ice%mu)
      alf_q = ice%a_vel * gfct((ice%nu+ice%b_vel+2.0_ireals)/ice%mu) / &
           gfct((ice%nu+2.0_ireals)/ice%mu)
      c_lam = gfct((ice%nu+1.0_ireals)/ice%mu)/gfct((ice%nu+2.0_ireals)/ice%mu)
      IF(isdebug) THEN
#ifdef SEDI_VECTORIZED
        WRITE (*,*)           "CLOUD ice_sedimentation_lm vectorized:"
#else
        WRITE (*,*)           "CLOUD ice_sedimentation_lm:"
#endif
        WRITE (*,'(A,ES10.3)') "  Coeff. lambda:          c_lam = ",c_lam
        WRITE (*,'(A,ES10.3)') "  Coeff. sedimentation n: alf_n = ",alf_n
        WRITE (*,'(A,ES10.3)') "  Coeff. sedimentation q: alf_q = ",alf_q
    ...
    WHERE (qi < 0.0_ireals) qi = 0.0_ireals
    WHERE (ni < 0.0_ireals) ni = 0.0_ireals
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          qi(i,j,k) = rho(i,j,k)*qi(i,j,k)
          ni(i,j,k) = rho(i,j,k)*ni(i,j,k)
          x_i = qi(i,j,k) / (ni(i,j,k)+eps)
          x_i = MIN(MAX(x_i,ice%x_min),ice%x_max)
! UB_20100107>>
          D_i = ice%a_geo * x_i**ice%b_geo 
          rhoexp = rhocorr(i,j,k)**(rhovelia+rhovelib*MIN(D_i,rhovelidknick))
! UB_20100107<<
          lam = ( c_lam * x_i )**(ice%b_vel) * rhoexp
          v_n = alf_n * lam
          v_q = alf_q * lam
          v_n = MAX(v_n,0.0_ireals)
          v_q = MAX(v_q,0.0_ireals)
          v_n = MIN(v_n,3.0_ireals)
          v_q = MIN(v_q,3.0_ireals)
          v_n_sedi(i,j,k) = -v_n
          v_q_sedi(i,j,k) = -v_q
...
#ifdef SEDI_VECTORIZED
    CALL sedi_lm_loop_vec (ni, qi, v_n_sedi, v_q_sedi, adz, dt, q_fluss, &
       &             ims,ime,jms,jme,kms,kme)
#else
    CALL sedi_lm_loop     (ni, qi, v_n_sedi, v_q_sedi, adz, dt, q_fluss, &
       &             ims,ime,jms,jme,kms,kme)
#endif
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          ...
          ni(i,j,k) = ni(i,j,k) / rho(i,j,k)
          rainrate(i,j,k) = - q_fluss(i,j,k) ! Cloud ice rate
        ENDDO
      ENDDO
    ENDDO
  ...
  SUBROUTINE rain_sedimentation_wrf (qr,qnr,qc,rainrate,rho,rhocorr,adz,dt, &
    ...
    ! .. Local Variables ..
    INTEGER  :: i, j, k, kk
    REAL     :: n_r,q_r,x_r,n0,lam,v_n,v_q,n,G1,G4,s_n,s_q,D_r,D_m,mue,rhoexp
    REAL :: c_n,c_q
    ...
    DO j = jts,jte
       DO k = kts,kte
          DO i = its,ite
             ...
             q_r = qr(i,k,j)                !..Fluessigwassergehalt in SI
             IF (cloud_typ <= 1) THEN
                lam = (pi*rho_w*n_0/q_r)**(1./4.)         !..Nur q_r  (n_0 = const) 
                D_r = 1.0/lam
                mue = 0.0
...
!!$ UB: that is not consistent with the original Lin and Rood (1996)...
  ...
  SUBROUTINE graupel_sedimentation_wrf (qg,qng,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER          :: i, j, k, kk
    INTEGER, SAVE    :: firstcall
    REAL :: x,n0,lam,v_n,v_q,n,s_n,s_q,rhoexp,D_g
...
! UB_20100107>>
             D_g = graupel%a_geo * x**graupel%b_geo 
             rhoexp = rhocorr(i,j,k)**(rhovelga+rhovelgb*MIN(D_g,rhovelgdknick))
! UB_20100107<<
             lam = ( c_lam * x )**(graupel%b_vel) * rhoexp
             v_n = alf_n * lam
             v_q = alf_q * lam
             v_n = MAX(v_n,0.1e+0)
             v_q = MAX(v_q,0.1e+0)
...
! UB_20080313<
  ...
  SUBROUTINE hail_sedimentation_wrf (qh,qnh,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER          :: i, j, k, kk
    INTEGER, SAVE    :: firstcall
    REAL :: x,n0,lam,v_n,v_q,n,s_n,s_q,cc,zz,rhoexp,D_h
    REAL, SAVE      :: alf_n,alf_q,c_lam
    REAL, PARAMETER :: eps = 1.00e-20
...
! UB_20100107>>
             D_h = hail%a_geo * x**hail%b_geo 
             rhoexp = rhocorr(i,j,k)**(rhovelha+rhovelhb*MIN(D_h,rhovelhdknick))
! UB_20100107<<
             lam = ( c_lam * x )**(hail%b_vel) * rhoexp
             v_n = alf_n * lam
             v_q = alf_q * lam
             v_n = MAX(v_n,0.1e+0)
             v_q = MAX(v_q,0.1e+0)
...
! UB_20080313<
  ...
  SUBROUTINE snow_sedimentation_wrf (qs,qns,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER          :: i, j, k, kk
    INTEGER, SAVE    :: firstcall
    REAL :: x,n0,lam,v_n,v_q,n,mue,nue,D_m,s_n,s_q,rhoexp
    REAL, SAVE      :: alf_n,alf_q,c_lam   
    REAL, PARAMETER :: eps = 1.00e-20   
...
! UB_20080313<
  ...
  SUBROUTINE ice_sedimentation_wrf (qi,qni,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER          :: i, j, k, kk
    INTEGER, SAVE    :: firstcall
    REAL :: x,n0,lam,v_n,v_q,n,s_n,s_q,D_i,rhoexp
    REAL, SAVE      :: alf_n,alf_q,c_lam   
    REAL, PARAMETER :: eps = 1.00e-20   
...
! UB_20100107>>
          D_i = ice%a_geo * x**ice%b_geo 
          rhoexp = rhocorr(i,j,k)**(rhovelia+rhovelib*MIN(D_i,rhovelidknick))
! UB_20100107<<
          lam = ( c_lam * x )**(ice%b_vel) * rhoexp
          v_n = alf_n * lam                                 !..Fallgeschw. fuer n
          v_q = alf_q * lam                                 !..Fallgeschw. fuer q
          v_n = MAX(v_n,0.0e+0)
          v_q = MAX(v_q,0.0e+0)
...
CONTAINS
  ...
  SUBROUTINE ice_nucleation()
    ...
    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, T, p, q, &
         &                        q_cloud,q_ice,q_rain,q_snow,q_graupel,          &
         &                        n_ice, n_snow, w,rho,dt,   &
         &                        s_i,s_w,dSidz,dT0dz,                            &
         &                        dqdt,speichere_dqdt,                &
    ...
    DOUBLE PRECISION            :: rho_a,q_d,e_d,dTdz_w,dSdz_w
    ...
    DO k = 1, loc_iz
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          IF (T(i,j,k) < T_nuc .and. s_i(i,j,k) > 0.0) THEN
            nuc_n = 0d0
            IF (nuc_typ < 8) THEN
              ndiag = n_ice_diagnostic(T(i,j,k),MIN(s_i(i,j,k),0.25d0),s_w(i,j,k),nuc_typ)
            ...
            ELSEIF (nuc_typ == 9 .AND. T(i,j,k) >= 245.15 .AND. &
                 (s_i(i,j,k) >= 0.08 .OR. (T(i,j,k) <= 265.16 .AND. s_w(i,j,k) >= 0.0))) THEN
              ! Zeitableitungsformulierung auf Basis der Cooper-Formel:
              dTdz_w = dT0dz(i,j,k)
              nuc_n = 5.0 * 0.304 * EXP( 0.304 * (T_3 - T(i,j,k)) ) * dTdz_w * dt
              nuc_n = max(nuc_n,0.d0)
            ENDIF
  ...
  SUBROUTINE ice_nucleation_vec()
    ...
    ! Locale Variablen 
    DOUBLE PRECISION            :: rho_a,q_d,e_d,dTdz_w,dSdz_w,sitmp,eitmp,ewtmp
    ...
    IF(isIO() .AND. isdebug)THEN
      IF (nuc_typ == 1) THEN
        WRITE (6, *) "CLOUD ice_nucleation_vec: fletcher-formel"
      ELSEIF (nuc_typ == 2) THEN
        WRITE (6, *) "CLOUD ice_nucleation_vec: meyers-formel" 
      ELSEIF (nuc_typ == 3) THEN
        WRITE (6, *) "CLOUD ice_nucleation_vec: cooper-formel" 
      ELSE
        WRITE (6, *) "CLOUD ice_nucleation_vec: nuc_typ = ",nuc_typ
      ENDIF
      WRITE (6,*) "CLOUD ice_nucleation_vec: max s_i    = ", maxval(s_i)
    END IF

    ALLOCATE (nuc_n(0:dim_ix, 1:dim_iy, 1:dim_iz), stat=stat_var)
    ALLOCATE (nuc_q(0:dim_ix, 1:dim_iy, 1:dim_iz), stat=stat_var)
          ...
          DO j = 1, loc_iy
            DO i = 0, loc_ix
              IF (T(i,j,k) < T_nuc .AND. s_i(i,j,k) > 0.0) THEN
                ...
                IF (sitmp >= 0.08 .OR. (T(i,j,k) <= 265.16 .AND. s_w(i,j,k) >= 0.0)) THEN
                  ndiag = n_ice_cooper( T(i,j,k) )
                ELSE
                  ndiag = 0.0d0
                END IF
                nuc_n(i,j,k) = MAX( ndiag - (n_ice(i,j,k)+n_snow(i,j,k)),0.d0)
          ...
          DO j = 1, loc_iy
            DO i = 0, loc_ix
              IF (T(i,j,k) < T_nuc .AND. s_i(i,j,k) > 0.0) THEN
                ...
                ewtmp = e_ws( T(i,j,k) )
                eitmp = e_es( T(i,j,k) )
                ndiag = n_ice_huffmann_vali_vec( T(i,j,k), sitmp, ewtmp, eitmp )
                nuc_n(i,j,k) = MAX( ndiag - (n_ice(i,j,k)+n_snow(i,j,k)),0.d0)
              END IF
            END DO
          END DO
      ...
      CASE(6)
        ...
        DO k = 1, loc_iz
          DO j = 1, loc_iy
            DO i = 0, loc_ix
              IF (T(i,j,k) < T_nuc .AND. s_i(i,j,k) > 0.0) THEN
                ndiag = n_ice_fletcher_contact( T(i,j,k) )
                nuc_n(i,j,k) = MAX( ndiag - (n_ice(i,j,k)+n_snow(i,j,k)),0.d0)
              END IF
            END DO
          END DO
    ...
    ELSEIF (nuc_typ == 8) THEN
      ...
      DO k = 1, loc_iz
        DO j = 1, loc_iy
          DO i = 0, loc_ix
            IF (T(i,j,k) < T_nuc .AND. s_i(i,j,k) > 0.0 .AND. dSidz(i,j,k) > 0.0) THEN
!              sitmp = MIN(s_i(i,j,k),0.25d0)
              sitmp = s_i(i,j,k)
              dSdz_w = dSidz(i,j,k)
    ...
    ELSEIF (nuc_typ == 9 .AND. T(i,j,k) >= 245.15 .AND. &
      ...
      DO k = 1, loc_iz
        DO j = 1, loc_iy
          DO i = 0, loc_ix
            IF (T(i,j,k) < T_nuc .AND. s_i(i,j,k) > 0.0 .AND. &
                 T(i,j,k) >= 245.15 .AND. &
                 (s_i(i,j,k) >= 0.08 .OR. (T(i,j,k) <= 265.16 .AND. s_w(i,j,k) >= 0.0)) &
                 ) THEN
              ! Zeitableitungsformulierung auf Basis der Cooper-Formel:
              dTdz_w = dT0dz(i,j,k)
              nuc_n(i,j,k) = 5.0 * 0.304 * EXP( 0.304 * (T_3 - T(i,j,k)) ) * dTdz_w * dt
              nuc_n(i,j,k) = MAX(nuc_n(i,j,k),0.d0)
            ENDIF
          ENDDO
        ENDDO
  ...
  DOUBLE PRECISION FUNCTION n_ice_huffmann_vali(T_a,S)
    ...
    e_w = e_ws(T_a)
    e_i = e_es(T_a)
!!$ UB_20121004    S_0 = e_i/e_w*(S+1.0)-1.0
!!$    S_0 = e_w/e_i-1.0
    S_0 = MAX(e_w/e_i,1.0d0+eps)-1.0
...
! AS_2010>>
  !***********************************************************************
  ! Based on paper 'An Empirical Parameterization of Heterogeneous 
  ! Ice Nucleation for Multiple Chemical Species of Aerosol'
  ! by Phillips et al. (JAS,2008)
  !***********************************************************************

  SUBROUTINE het_nuc_phillips(T_in,p_in,qv_in,fractions)
  
! UB_20100425>>
    USE wolken_driver, ONLY : pi
! UB_20100425<<

    IMPLICIT NONE

!! UB_20100416:
!!$    REAL(KIND=8)  , INTENT(in)  :: T_in,p_in,qv_in
!!$    REAL(KIND=8), DIMENSION(3) :: fractions
!!$    REAL(KIND=8)  :: fractions

    DOUBLE PRECISION  , INTENT(in)  :: T_in,p_in,qv_in
    DOUBLE PRECISION                :: fractions(3)
! UB<<

    REAL :: num_ic_dust_imm, n_dust ,num_ic_soot_imm, n_soot, &
         num_ic_organic_imm, num_organic,hlp

    REAL:: RHO_CFDC, BASE_DUST_OMEGA, BASE_SOOT_PHILIC_OMEGA, &
         BASE_ORGANIC_PHOBIC_OMEGA, ALPHA_DUST, &
         ALPHA_SOOT, ALPHA_ORGANIC, FRACTION_DEPNUCL_WARM_DUST
    INTEGER :: IMM_COND_ON, OTHER_SCHEME, &
         SIZE_RESOLVED, MAX_GRID

    ! Email Vaughan Phillips vom 17 Feb 2010:
    ! the baseline dust surface mixing ratio (Omega_DM_star etc) should be 4 times 
    ! the value published: it ought to be 2E-6 instead of 5E-7 SI units.

    PARAMETER(&
         OTHER_SCHEME = 0, &
         IMM_COND_ON = 0,       &  
         BASE_DUST_OMEGA = 2.0E-6, &    !5.0e-7, &
         BASE_SOOT_PHILIC_OMEGA = 2.7e-7, &  
         BASE_ORGANIC_PHOBIC_OMEGA = 9.1e-7, &  
         ALPHA_DUST = 2./3., &           
         ALPHA_SOOT = 1./3. - 0.06, &    
         ALPHA_ORGANIC = 0.06, & 
         RHO_CFDC = 50000./(287.*228.15), &
         FRACTION_DEPNUCL_WARM_DUST = 0.15, &
         SIZE_RESOLVED = 1, &
         MAX_GRID = 10000)

    REAL:: FAC_CORRECT_RH = 2., A_organic_2um
    REAL:: dN, D, n_zero, EPS = 0.622 
    REAL:: diam_aero_modal, sigma_aero, n_frozen, sum_n, arg1, arg3, arg4, &
         dnbydlogd_normalised, fac = 1.001, mu, D_mean_IN, & 
         n_grid_dust(MAX_GRID), D_grid_dust(MAX_GRID), &
         A_grid_dust(MAX_GRID), n_IN_grid_dust(MAX_GRID), &
         n_grid_soot(MAX_GRID), D_grid_soot(MAX_GRID), &
         n_grid_organic(MAX_GRID), D_grid_organic(MAX_GRID)

    REAL:: n_in,tc2,qcb,QSI,S_w

    REAL::  sum_A, sum_q, sum_q2,sum_D, QSW, P_SAT, &
         n_in_dust, n_in_ultra, n_in_dust_ultra, A_dust, sum_A1, sum_A_2um, &
         CIHENC_dust, ESW, ESI, SS_i, n_iw, nin_a_nuc_dust, &
         bulk_density, Q_soot, Q_dust, Q_SO4,A_soot,  &
         Q_organic, A_organic_phobic, nin_a_nuc_soot, nin_a_nuc_organic, n_in_soot_ultra, &
         Psi_dust, Psi_soot, Psi_organic, &          ! factor for rescaling Meyers eq.
         H_frac_soot, H_frac_organic, H_frac_dust, & ! fraction reducing IN activity at low Si,warm T
         n_in_soot, n_in_organic, n_in_organic_ultra, &
         CIHENC_soot, CIHENC_organic, DTEMP, &
         delta_Si, delta_T, delta_Sw, n_in_max, SS_iw, n_dust_zero, n_soot_zero, &
         S_i_0, tc_HM_degC, rho_MTWERNER, tc2_dum, sum_Dm,  S_w_0, dep_frac, n_in_hat, n_in_tilde, &
         dNbydlogD_1, dNbydlogD_2, n_zero_1, sigma_aero_1, diam_aero_modal_1, n_zero_2, sigma_aero_2, &
         diam_aero_modal_2, dNbydlogD_norm, D_save

    INTEGER ::ij, ijstop_dust,  ijstop_soot,  ijstop_organic 

    LOGICAL, PARAMETER :: debug = .FALSE.

    rho_MTWERNER = 68800/(287*(273.15-2.4))

    IF(OTHER_SCHEME > -1) THEN
       Q_organic = 3.e-9     ! Fig. 4  Clarke et al. (2007) [kg/kg]
       Q_soot = 0.1*Q_organic
       Q_SO4 =  0.1e-9
    ELSE
       Q_soot = 3.e-11
       Q_organic = 0.8e-10   ! Value of mixing ratio for aerosols smaller than 2.5 mu [kg/kg]             
    ENDIF
    !Q_dust = 30.*0.25e-9/rho_MTWERNER
    Q_dust = 0.25e-9/rho_MTWERNER


    n_zero_1 = 1.e9
    n_zero_2 = n_zero_1/150.

    sigma_aero_1 = 1.9          
    diam_aero_modal_1 = 0.8e-6  

    sigma_aero_2 = 1.6          
    diam_aero_modal_2 =  3.e-6  

    bulk_density = 2300.        

    D = 0.01e-6; n_frozen = 0.; sum_n = 0.; sum_q = 0.; sum_D = 0.; sum_A = 0.; sum_Dm = 0.; ij = 0;  sum_q2 = 0.;

    DO WHILE (D < 10.e-6)
       arg1 = n_zero_1/(SQRT(2.* pi)*LOG10(sigma_aero_1));
       arg3 = (LOG10(D/diam_aero_modal_1))**2.;
       arg4 = 2.*((LOG10(sigma_aero_1))**2.);
       dNbydlogD_norm = arg1*EXP(-arg3/arg4);
       arg1 = n_zero_2/(SQRT(2.* pi)*LOG10(sigma_aero_2));
       arg3 = (LOG10(D/diam_aero_modal_2))**2.;
       arg4 = 2.*((LOG10(sigma_aero_2))**2.);
       dNbydlogD_norm = dNbydlogD_norm + arg1*EXP(-arg3/arg4);

       IF(D_save < diam_aero_modal_1 .AND. D > diam_aero_modal_1) dNbydlogD_1 = dNbydlogD_norm
       IF(D_save < diam_aero_modal_2 .AND. D > diam_aero_modal_2) dNbydlogD_2 = dNbydlogD_norm

       dN = dNbydlogD_norm*LOG10(fac)

       sum_n = sum_n + dN
       sum_D = sum_D + dN*D
       sum_Dm = sum_Dm + dN* pi *D*D

       IF(D > 0.1e-6) THEN 
          sum_A = sum_A + dN*4.* pi *((D/2.)**2.)
          ij = ij + 1
          n_grid_dust(ij) = dN
          D_grid_dust(ij) = D
          A_grid_dust(ij) = dN*4.* pi *((D/2.)**2.)
       ENDIF

       IF(D < 2.5e-6) sum_q = sum_q + dN * (4./3.)* pi *((D/2.)**3.)*bulk_density
       sum_q2 = sum_q2 + dN * (4./3.)* pi *((D/2.)**3.)*bulk_density
    ...
    IF(ijstop_dust > MAX_GRID) STOP 32145

    n_dust = Q_dust * sum_n/sum_q
    sum_q2 = Q_dust * sum_q2/sum_q
    n_grid_dust = n_grid_dust * Q_dust/sum_q
    A_grid_dust = A_grid_dust * Q_dust/sum_q

    A_dust = Q_dust * sum_A/sum_q

    n_zero = 1.e9
    diam_aero_modal = 0.2e-6
    sigma_aero = 1.6
    bulk_density = 1860. 

    D = 0.01e-6; n_frozen = 0.; sum_n = 0.; sum_q = 0.; sum_D = 0.; sum_A = 0.; sum_A1 = 0.; ij = 0;
    DO WHILE (D < 10.e-6)


       arg1 = n_zero/(SQRT(2.* pi)*LOG10(sigma_aero))
       arg3 = (LOG10(D/diam_aero_modal))**2.

       arg4 = 2.*(LOG10(sigma_aero)**2.)
       dNbydlogD_normalised = arg1*EXP(-arg3/arg4)
       dN = dNbydlogD_normalised*LOG10(fac) 

       sum_n = sum_n + dN
       sum_D = sum_D + dN*D
       IF(D > 0.1e-6) THEN 
          sum_A = sum_A + dN*4.* pi *((D/2.)**2.)
          IF(D < 1.e-6) sum_A1 = sum_A1 + dN*4.* pi *((D/2.)**2.)
          ij = ij + 1
          n_grid_soot(ij) = dN
          D_grid_soot(ij) = D
    ...
    IF(ijstop_soot > MAX_GRID) THEN
       WRITE (*,*) ' ERROR: MAX_GRID too small, ijstop_soot = ',ijstop_soot
       STOP
    END IF


    n_soot = Q_soot * sum_n/sum_q
    n_grid_soot = n_grid_soot * Q_soot/sum_q

    A_soot = Q_soot * sum_A/sum_q
    ...
    sum_A1 = 0.; D = 0.01e-6; n_frozen = 0.; sum_n = 0.; sum_q = 0.; sum_D = 0.; sum_A = 0.; sum_Dm = 0.; sum_A_2um = 0.;
    ij = 0
    DO WHILE (D < 10.e-6)
       ...
       sum_D = sum_D + dN*D
       sum_n = sum_n + dN
       sum_Dm = sum_Dm + dN*D*(4./3.)* pi *((D/2.)**3.)*bulk_density
       IF(D > 0.1e-6) THEN
          sum_A = sum_A + dN*4.* pi *((D/2.)**2.)
          IF(D < 1.e-6) sum_A1 = sum_A1 + dN*4.* pi *((D/2.)**2.)
          ij = ij + 1
          n_grid_organic(ij) = dN
          D_grid_organic(ij) = D

       ENDIF
       IF(D < 2.5e-6)sum_q = sum_q + dN * (4./3.)* pi *((D/2.)**3.)*bulk_density
       IF(D > 0.1e-6 .AND. D < 1.e-6) sum_A_2um = sum_A_2um + dN*4.* pi *((D/2.)**2.)

       D = D*fac
    ENDDO
    ijstop_organic = ij
    IF(ijstop_organic > MAX_GRID) STOP 32146

    num_organic = Q_organic* sum_n/sum_q
    n_grid_organic = n_grid_organic * Q_organic/sum_q

    A_organic_phobic = Q_organic * sum_A/sum_q
    A_organic_2um = Q_organic * sum_A_2um/sum_q
    ...
    ESI = e_es(T_in)
    QSI = EPS*ESI/(p_in-ESI)
    ESW = e_ws(T_in)
    QSW = EPS*ESW/(p_in-ESW)

    IF (IMM_COND_ON == 1 .OR. qv_in.lt.0) THEN
       qcb = QSW         ! water vapor mixing ratio
    ELSE
       qcb = qv_in
       IF(qcb > QSW*0.97) qcb = QSW*0.97
!AS201002       IF(qcb > QSW*0.97) qcb = QSW*0.97 ! This line does make a difference!
!       IF(qcb > QSW*0.99) qcb = QSW*0.99
!       IF(qcb < QSI)      qcb = QSI + 1e-20
    ...
    IF(qcb >  QSI) THEN
       IF(tc2 < 273.15 .AND. tc2 > 273.15 - 90. ) THEN
          ...
          IF(OTHER_SCHEME <= 0) THEN


             delta_Si = H_1_smooth(SS_i + 1, 1.1, 1.2, 0., 1.);                              ! Range in Si for transition of Hx                  
             delta_T = H_1_smooth(-(tc2-273.15), 35., 40., FRACTION_DEPNUCL_WARM_DUST, 1.);  ! Range in T for transition of Hx near Tx,0
             delta_Sw =  H_1_smooth(qcb/QSW, 0.97, 1., 0., 1.);

             tc_HM_degC = tc2 - 273.15                


             S_i_0 = 1. + 10.**(8.2584e-6*tc_HM_degC*tc_HM_degC*tc_HM_degC + 5.3938E-4*tc_HM_degC*tc_HM_degC &
                  + 3.1656E-3*tc_HM_degC - 1.0261) !Threshold of Si in Hx for X=DM; w/ interpolation variables
             ! PRINT *,'S_i_0 = ', S_i_0, tc_HM_degC

             S_w_0 = 0.97                          ! Threshold of Sw in Hx

             dep_frac = H_1_smooth(SS_i + 1, S_i_0,  S_i_0 + 0.1, 0.,1.)* &
                  H_1_smooth(-(tc2-273.15), 35., 40., FRACTION_DEPNUCL_WARM_DUST, 1.)/FAC_CORRECT_RH ! Eq.12 P08	

             H_frac_dust = dep_frac  + (1. - dep_frac)*H_1_smooth(qcb/QSW, S_w_0, 1., 0.,1.)         ! Eq.11 P08 
             ...
             S_w_0 = 0.97                          ! Threshold of Sw in Hx
             tc2_dum = tc2                         ! Temperature [K]
             IF(tc2_dum > 239.) tc2_dum = 239.
             IF(tc2_dum < 198.) tc2_dum = 198.

             S_i_0 = -3.118E-5*tc2_dum*tc2_dum + 1.085e-2*tc2_dum + 5.652e-1     !Threshold of Si in Hx for X=BC

             dep_frac = H_1_smooth(SS_i + 1, S_i_0-0.1, S_i_0, 0.,1.)* H_1_smooth(-(tc2-273.15), 45., 50., 0.,1.) &
                  /FAC_CORRECT_RH                                                                    ! Eq.12 P08
             H_frac_soot = dep_frac  + (1. - dep_frac)*H_1_smooth(qcb/QSW, S_w_0, 1., 0.,1.)         ! Eq.11 P08 
             ...
             IF(tc2 < 273.15 .AND. tc2 >= 273.15 - 35.) THEN
                n_in = 1.E3* (EXP(12.96*SS_i - 0.639)/RHO_CFDC) *0.0587*FAC_CORRECT_RH               ! Eq. 3 P08 
                ...
                IF(tc2 < 273.15 -25. ) THEN
                   ...
                   ! Eq.6 P08 describing het. nucleation in background-trop. scenarion at water saturation 
                ...
                ELSE
                   n_in_dust = 0.
                   D_mean_IN = 0.; sum_n = 0.;  n_IN_grid_dust = 0.;
                   DO ij = 1, ijstop_dust
                      mu = n_in*ALPHA_DUST*H_frac_dust* pi *D_grid_dust(ij)*D_grid_dust(ij)&   ! Eq.10 P08
                           /BASE_DUST_OMEGA

                      n_in_dust = n_in_dust + (1. - EXP(-mu))*n_grid_dust(ij)                  ! Eq.9 P08

                      D_mean_IN = D_mean_IN + (1. - EXP(-mu))*n_grid_dust(ij)*D_grid_dust(ij)
                      n_IN_grid_dust(ij) = (1. - EXP(-mu))*n_grid_dust(ij)
                      sum_n = sum_n + (1. - EXP(-mu))*n_grid_dust(ij)

                   ENDDO
                   IF(sum_n > 0.) THEN
                      D_mean_IN = D_mean_IN / sum_n
                   ELSE 
                      D_mean_IN = 0.
                   ENDIF
                ENDIF
                ...
                ELSE
                   n_in_soot = 0.
                   DO ij = 1, ijstop_soot
                      mu = n_in*ALPHA_SOOT*H_frac_soot* pi *(D_grid_soot(ij)**2.) &            ! Eq.10 P08
                           /BASE_SOOT_PHILIC_OMEGA
                      n_in_soot = n_in_soot + (1. - EXP(-mu))*n_grid_soot(ij)                  ! Eq.9 P08
                   ENDDO
                ENDIF
                IF(SIZE_RESOLVED == 0) THEN
                ...
                ELSE
                   n_in_organic = 0.
                   DO ij = 1, ijstop_organic
                      mu = n_in*ALPHA_ORGANIC*H_frac_organic* pi *(D_grid_organic(ij)**2.) &   ! Eq.10 P08
                           /BASE_ORGANIC_PHOBIC_OMEGA
                      n_in_organic = n_in_organic + (1. - EXP(-mu))*n_grid_organic(ij)         ! Eq.9 P08
                   ENDDO
             ...
             IF(tc2 < 273.15 - 35.) THEN
                ...
                ELSE
                   n_in_dust_ultra = 0.; sum_n = 0.; D_mean_IN = 0.; n_IN_grid_dust = 0.
                   DO ij = 1, ijstop_dust
                      mu = n_in_ultra*ALPHA_DUST*H_frac_dust* pi *(D_grid_dust(ij)**2.) &      ! Eq.10 P08
                           /BASE_DUST_OMEGA
                      n_in_dust_ultra = n_in_dust_ultra + (1. - EXP(-mu))*n_grid_dust(ij)

                      D_mean_IN = D_mean_IN + (1. - EXP(-mu))*n_grid_dust(ij)*D_grid_dust(ij)
                      n_IN_grid_dust(ij) = (1. - EXP(-mu))*n_grid_dust(ij)                     ! Eq.9 P08
                      sum_n = sum_n + (1. - EXP(-mu))*n_grid_dust(ij)
                ...
                ELSE
                   n_in_soot_ultra = 0
                   DO ij = 1, ijstop_soot
                      mu = n_in_ultra*ALPHA_SOOT*H_frac_soot*pi*(D_grid_soot(ij)**2.) &
                           /BASE_SOOT_PHILIC_OMEGA
                ...
                ELSE
                   n_in_organic_ultra = 0.
                   DO ij = 1, ijstop_organic
                      mu = n_in_ultra*ALPHA_ORGANIC*H_frac_organic*pi*(D_grid_organic(ij)**2.) &
                           /BASE_ORGANIC_PHOBIC_OMEGA
                      n_in_organic_ultra = n_in_organic_ultra + (1. - EXP(-mu))*n_grid_organic(ij)
                   ENDDO
    ...
    fractions(2) = DBLE(num_ic_soot_imm)/DBLE(n_soot)
    fractions(3) = DBLE(num_ic_organic_imm)/DBLE(num_organic)
    fractions(1) = DBLE(num_ic_dust_imm)/DBLE(n_dust)
    
    IF (debug) THEN 
       WRITE (*,'(2x,2a5,3(2x,a,f6.2),4(2x,a,f8.4))') ' ',' ',&
            ' T = ',tc2,  &
            'Si = ',SS_i, &
            'Sw = ',S_w, &
            'IFRC_dust = ',num_ic_dust_imm/n_dust, &
            'IFRC_soot = ',num_ic_soot_imm/n_soot, &
            'IFRC_orga = ',num_ic_organic_imm/num_organic
       WRITE (*,'(2x,2a5,3(2x,a,f6.2),4(2x,a,f8.4))') ' ',' ',&
            ' T = ',tc2,  &
            'Si = ',SS_i, &
            'Sw = ',S_w, &
            'IFRC_dust = ',fractions(1), &
            'IFRC_soot = ',fractions(2), &
            'IFRC_orga = ',fractions(3)
       WRITE (*,'(2x,2a5,3(2x,a,f6.2),4(2x,a,e10.3))') ' ',' ',&
            ' T = ',tc2,  &
            'Si = ',SS_i, &
            'Sw = ',S_w, &
            'n_dust = ',n_dust, &
    ...
    END IF
    IF (debug) THEN 
       WRITE (*,'(2(2x,a,f6.2),6(2x,a,e10.3))') &
            ' T = ',tc2,  &
            'Si = ',qcb/QSI, &
            'IN_dust = ',num_ic_dust_imm, 'IFRC_dust = ',num_ic_dust_imm/n_dust, &
            'IN_soot = ',num_ic_soot_imm, 'IFRC_soot = ',num_ic_soot_imm/n_soot, &
            'IN_organic = ',num_ic_organic_imm, 'IFRC_organic = ',num_ic_organic_imm/num_organic
    ENDIF

  END SUBROUTINE het_nuc_phillips

  !***********************************************************************
  ! Function H_1_smooth
  !***********************************************************************

  REAL ELEMENTAL FUNCTION H_1_smooth(X, X1, X2, Hlo, Hhi)
    IMPLICIT NONE
    REAL, INTENT(in) :: Hlo, Hhi, X, X1, X2
    REAL :: X_1, X_2, a_0, a_1, a_2, a_3, A, B
  ...
  SUBROUTINE ice_nucleation_homhet()
    ...
    DOUBLE PRECISION  :: Si, e_si
    DOUBLE PRECISION  :: ni_hom,ri_hom,mi_hom
    DOUBLE PRECISION  :: v_th,n_sat,flux,phi,D_v,cool,tau,delta,w_p,scr
    DOUBLE PRECISION  :: ctau, tau_g,acoeff(3),bcoeff(2), ri_dot
    DOUBLE PRECISION  :: kappa,sqrtkap,ren,R_imfc,R_im,R_ik,ri_0
    DOUBLE PRECISION  :: tgrow,ri_max,beta,xj,dxj,xmid,fmid,nimax
    DOUBLE PRECISION  :: xt,xs
    INTEGER       ::  jj,ss,tt

    LOGICAL :: use_hetnuc, use_homnuc, use_wp

    DOUBLE PRECISION, DIMENSION(3) :: infrac

    DOUBLE PRECISION, ALLOCATABLE :: nuc_n(:,:,:), nuc_q(:,:,:)
! UB_20100420>>
    DOUBLE PRECISION  :: nuc_n_tmp, nuc_q_tmp
! UB_20100420<<
    ...
    IF(isIO())THEN
      IF (use_hetnuc .AND. .NOT.use_homnuc) THEN
        WRITE (*, *) "CLOUD ice_nucleation: Heterogeneous nucleation only"
      ELSE IF (.NOT.use_hetnuc .AND. use_homnuc) THEN
        WRITE (*, *) "CLOUD ice_nucleation: Homogeneous nucleation only (KHL06)"
      ELSE IF ( use_hetnuc .AND. use_homnuc .AND. .NOT.use_wp ) THEN
        WRITE (*, *) "CLOUD ice_nucleation: Homog. and het. nucleation, but wp = 0 in KHL06"
      ELSE IF ( use_hetnuc .AND. use_homnuc .AND. use_wp ) THEN
        WRITE (*, *) "CLOUD ice_nucleation: Homog. and het. nucleation, using KHL06 scheme"
      END IF
    END IF
        ...
        DO k = 1, loc_iz
          DO j = 1, loc_iy
            DO i = 0, loc_ix
              ...
              IF (T(i,j,k) < T_nuc .AND. s_i(i,j,k) > 0.0 .AND. dSidz(i,j,k) > 0.0 &
                   & .AND. ( n_ice(i,j,k)+n_snow(i,j,k) < ni_het_max ) )THEN

! UB_20100420>> Umgestellt auf Aufsummierung der nuc_n, nuc_q wg. Umwandlungsratenausgabe:
!!$                nuc_n(i,j,k) =  N_m0 * EXP( a_md + b_md * s_i(i,j,k) ) * b_md * dSidz(i,j,k) * dt
                nuc_n_tmp =  N_m0 * EXP( a_md + b_md * s_i(i,j,k) ) * b_md * dSidz(i,j,k) * dt

!!$                nuc_n(i,j,k) = MAX(nuc_n(i,j,k), 0.d0)
!!$                nuc_q(i,j,k) = MIN(nuc_n(i,j,k) * ice%x_min, q(i,j,k))

                nuc_n_tmp = MAX(nuc_n_tmp, 0.d0)
                nuc_n(i,j,k) = nuc_n(i,j,k) + nuc_n_tmp
                nuc_q_tmp = MIN(nuc_n_tmp * ice%x_min, q(i,j,k))
                nuc_q(i,j,k) = nuc_q(i,j,k) + nuc_q_tmp

!!$                n_ice(i,j,k) = n_ice(i,j,k) + nuc_n(i,j,k)
!!$                q_ice(i,j,k) = q_ice(i,j,k) + nuc_q(i,j,k)
!!$                q(i,j,k)     = q(i,j,k)     - nuc_q(i,j,k)

                n_ice(i,j,k) = n_ice(i,j,k) + nuc_n_tmp
                q_ice(i,j,k) = q_ice(i,j,k) + nuc_q_tmp
                q(i,j,k)     = q(i,j,k)     - nuc_q_tmp
! UB_20100420<<
      ...
      ELSE
        ...
        ! for nuc_typ == 4, the default values from module
	! wolken_konstanten are used. Otherwise the following
	! changes apply:
...
! UB_20100420>> Umgestellt auf Aufsummierung der nuc_n, nuc_q wg. Umwandlungsratenausgabe:
!!$                nuc_n(i,j,k) = MAX( ndiag - (n_ice(i,j,k)+n_snow(i,j,k)),0.d0)
                nuc_n_tmp = MAX( ndiag - (n_ice(i,j,k)+n_snow(i,j,k)),0.d0)
                nuc_n(i,j,k) = nuc_n(i,j,k) + nuc_n_tmp
...
! UB_20100420>> Umgestellt auf Aufsummierung der nuc_n, nuc_q wg. Umwandlungsratenausgabe:
!!$                nuc_n(i,j,k) = MAX(MIN(ni_hom, ni_hom_max), 0.d0)
!!$                nuc_q(i,j,k) = MIN(nuc_n(i,j,k) * mi_hom, q(i,j,k))

                nuc_n_tmp = MAX(MIN(ni_hom, ni_hom_max), 0.d0)
                nuc_q_tmp = MIN(nuc_n_tmp * mi_hom, q(i,j,k))
                nuc_n(i,j,k) = nuc_n(i,j,k) + nuc_n_tmp
                nuc_q(i,j,k) = nuc_q(i,j,k) + nuc_q_tmp
...
! AS_2010<<
  ...
  SUBROUTINE ice_cloud_riming()
    ...
    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, &
                                  T, p, q,  &
      ...
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          ...
          IF (q_c > q_krit_c .AND. q_i > q_krit_ic .AND. D_i > D_krit_ic .AND. D_c > D_krit_c) THEN
            ...
            v_i = ice%a_vel   * x_i**ice%b_vel   * &
                 rrho_04(i,j,k)**(rhovelia+rhovelib*MIN(D_i,rhovelidknick)) !..mittlere Sedimentationsgeschw.
...
!!! UB_20090724: Wie bei ice_rain_riming sollte das folgende eigentlich nur dann geschehen, wenn
!!! T_a < T_3. Andernfalls sollte das eingesammelte Wolkenwasser wieder abgeschedded werden
!!! oder stillschweigend als zum Schnee gehoerig angesehen werden (was beides
!!! seine Vor- und Nachteile bzw. Fehler hat)!
      ...
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          ...
          IF (q_c > q_krit_c .AND. q_s > q_krit_sc .AND. D_s > D_krit_sc .AND. D_c > D_krit_c) THEN
            ...
            v_c = cloud%a_vel * x_c**cloud%b_vel * rrho_c(i,j,k)  !..mittlere Sedimentationsgeschw.
            v_s = snow%a_vel  * x_s**snow%b_vel  * &
                 rrho_04(i,j,k)**(rhovelsa+rhovelsb*MIN(D_s,rhovelsdknick)) !..mittlere Sedimentationsgeschw.
...
!!! UB_20090724: Wie bei snow_rain_riming sollte das folgende eigentlich nur dann geschehen, wenn
!!! T_a < T_3. Andernfalls sollte das eingesammelte Wolkenwasser wieder abgeschedded werden
!!! oder stillschweigend als zum Schnee gehoerig angesehen werden (was beides
!!! seine Vor- und Nachteile bzw. Fehler hat)!
  ...
  SUBROUTINE graupel_cloud_riming()
    ...
    ALLOCATE(riming_n(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
    ALLOCATE(riming_q(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
    ALLOCATE(rate_q(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
      ...
      DO j = 1, loc_iy
        DO i = 0, loc_ix             
          ...
          IF (q_c > q_krit_c .AND. q_g > q_krit_gc .AND. D_g > D_krit_gc .AND. &
            ...
            v_g = graupel%a_vel * x_g**graupel%b_vel * &
                 rrho_04(i,j,k)**(rhovelga+rhovelgb*MIN(D_g,rhovelgdknick)) !..mittlere Sedimentationsgeschw.
            v_c = cloud%a_vel   * x_c**cloud%b_vel   * rrho_c(i,j,k)  !..mittlere Sedimentationsgeschw.
...
! ub>> Subroutine, die nur die Rate, mit der Wolkentropfen von Graupeln eingesammelt werden,
!      berechnet und ausgibt. Wird fuer "graupel to snow" 
!      in vapor_deposition_growth() bzw. vapor_dep_simple() gebraucht.
!      Die Rate wird mit dt vormultipliziert!
  SUBROUTINE graupel_cloud_riming_rate(rime_q)
    !*******************************************************************************
    !                                                                              *
    !       Berechnung des Bereifens der Graupelpartikel                           *
    !                                                                              *
    ...
    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, &
         ...
         &                        q_graupel, n_graupel, q_cloud, n_cloud, q_rain, n_rain,  &
         &                        q_ice, n_ice, dt
    USE konstanten,         ONLY: pi,wolke_typ
    USE parallele_umgebung, ONLY: isIO

    IMPLICIT NONE

    DOUBLE PRECISION, DIMENSION(0:dim_ix,1:dim_iy,1:dim_iz), INTENT(out)    :: rime_q

    ! Locale Variablen 
    INTEGER                     :: i,j,k
    INTEGER, SAVE               :: firstcall
    DOUBLE PRECISION            :: q_g,n_g,x_g,d_g,v_g
    DOUBLE PRECISION            :: q_c,n_c,x_c,d_c,v_c
    DOUBLE PRECISION            :: e_coll_q
    DOUBLE PRECISION, SAVE      :: delta_n_gg,delta_q_gc,delta_q_cc
    DOUBLE PRECISION, SAVE      :: theta_n_gg,theta_q_gc,theta_q_cc
    ...
    IF (isIO() .AND. isdebug) THEN
      WRITE (6, *) "CLOUD graupel_cloud_riming_rate (nur Rate berechnen)" 
    END IF
      ...
      DO j = 1, loc_iy
        DO i = 0, loc_ix             
          ...
          IF (q_c > q_krit_c .AND. q_g > q_krit_gc .AND. D_g > D_krit_gc .AND. &
            ...
            e_coll_q = MIN(e_gc, MAX(const1*(D_c - D_krit_c),e_min))


            rime_q(i,j,k) = pi/4.0 * e_coll_q * n_g * q_c * dt & 
                 &   * (delta_n_gg * D_g*D_g + delta_q_gc * D_g*D_c + delta_q_cc * D_c*D_c) &
                 &   * sqrt(theta_n_gg * v_g*v_g - theta_q_gc * v_g*v_c + theta_q_cc * v_c*v_c)

            rime_q(i,j,k) = MIN(q_c,rime_q(i,j,k))
  ...
  END SUBROUTINE graupel_cloud_riming_rate
      ...
      DO j = 1, loc_iy
        DO i = 0, loc_ix             
          ...
          IF (q_c > q_krit_c .AND. q_h > q_krit_hc .AND. D_h > D_krit_hc .AND. &
            ...
            v_h = hail%a_vel * x_h**hail%b_vel * &
                 rrho_04(i,j,k)**(rhovelha+rhovelhb*MIN(D_h,rhovelhdknick)) !..mittlere Sedimentationsgeschw.
            v_c = cloud%a_vel   * x_c**cloud%b_vel   * rrho_c(i,j,k)  !..mittlere Sedimentationsgeschw.
      ...
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          ...
          IF (q_r > q_krit .AND. q_i > q_krit_ir .AND. D_i > D_krit_ir) THEN
            ...
            v_r = rain%a_vel * x_r**rain%b_vel * &
                 rrho_04(i,j,k)**(rhovelra+rhovelrb*MIN(D_r,rhovelrdknick))  !..mittlere Sedimentationsgeschw.
...
#ifdef SAVE_CONVERSIONRATES
                  IF (speichere_dqdt) THEN
                    dqdt(i,j,k,29) = dqdt(i,j,k,29) + rime_qi
                  END IF
#endif
                END IF
              ELSE
...
#ifdef SAVE_CONVERSIONRATES
                  IF (speichere_dqdt) THEN
                    dqdt(i,j,k,30) = dqdt(i,j,k,30) + rime_qs
                  END IF
#endif
                END IF
              ELSE
...
! ub>> Subroutine, die nur die Rate, mit der Regentropfen von Graupeln eingesammelt werden,
!      berechnet und ausgibt. Wird fuer "graupel to snow"
!      in vapor_deposition_growth() bzw. vapor_dep_simple() gebraucht.
!      Die Rate wird mit dt vormultipliziert!
  SUBROUTINE graupel_rain_riming_rate(rime_q)
    !*******************************************************************************
    !                                                                              *
    !       Berechnung des Bereifens der Graupelpartikel                           *
    !                                                                              *
    ...
    DOUBLE PRECISION            :: q_g,n_g,x_g,d_g,v_g,d_h,x_h,d_gd
    DOUBLE PRECISION            :: q_r,n_r,x_r,d_r,v_r,q_h,n_h,d_rd
    DOUBLE PRECISION, SAVE      :: delta_n_gg,delta_q_gr,delta_q_rr
    DOUBLE PRECISION, SAVE      :: theta_n_gg,theta_q_gr,theta_q_rr

    DOUBLE PRECISION, PARAMETER :: eps  = 1.d-20

    IF (isIO() .AND. isdebug) THEN
        WRITE (6, *) "CLOUD graupel_rain_riming_rate" 
    END IF
      ...
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          ...
          IF (q_r > q_krit .AND. q_g > q_krit) THEN
            ...
            rime_q(i,j,k) = pi/4.0 * n_g * q_r * dt & 
                 &   * (delta_n_gg * D_g**2 + delta_q_gr * D_g*D_r + delta_q_rr * D_r**2) &
                 &   * (theta_n_gg * v_g**2 - theta_q_gr * v_g*v_r + theta_q_rr * v_r**2)**0.5
            
            rime_q(i,j,k) = MIN(q_r,rime_q(i,j,k))
  ...
  END SUBROUTINE graupel_rain_riming_rate
    ...
    DO k = 1, loc_iz
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          ...
          IF (deprate_snow(i,j,k) <= 0.0d0 .OR. deprate_snow(i,j,k) < rimeqcrate_snow(i,j,k)+rimeqrrate_snow(i,j,k)) THEN
...
! UB_20081125:            D_T  = K_T / (cp * rho(i,j,k))))
      ...
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          ...
          IF (T_a < T_3) THEN
            ...
            n_c = n_cloud(i,j,k)
            T_c = T_a - T_3
! UB_20091111>> shift down T for cloud_freeze by <tshift_cloudfreeze> K:
            T_c = MIN(T_c - tshift_cloudfreeze, 0.0d0)
! UB_20091111<<
            IF (q_c > 0.0) THEN
              IF (T_c < -50.0) THEN            
...
! AS_2010>> Very important limiting of cloud freezing in case of assumed
!           constant cloud number density; otherwise, abundant number of ice particles
!           may artificially be generated again and again ...
...
! ub<<
  ...
  SUBROUTINE rain_freeze_gamlook ()
    ...
    IF (firstcall.NE.1) THEN
      ...
      ! ordinary gamma function of nm3 is the last value in the lookup table 3:
      g3 = ltable3%igf(ltable2%n)
      coeff_z = g3 * g1 / g2**2
      IF (isIO() .AND. isdebug) THEN
...
! UB_20091111>> shift down T for cloud_freeze by <tshift_cloudfreeze> K:
            T_a = MIN(T_a - tshift_cloudfreeze, T_freeze)
! UB_20091111<<
...
!              WRITE (*,'(a,3(i3,x),f8.3,4(x,es10.3))') 'SEIFERT_PP RAIN_FREEZE_GAMLOOK: Qrain < 0.0 corrected, ', &
!                   i,j,k, T_a, q_r, j_het, fr_q, fr_q_tmp
              q_rain(i,j,k) = 0.0d0
            END IF
            IF (n_rain(i,j,k) < 0.0) THEN
!              WRITE (*,'(a,3(i3,x),f8.3,4(x,es10.3))') 'SEIFERT_PP RAIN_FREEZE_GAMLOOK: Nrain < 0.0 corrected, ', &
!                   i,j,k, T_a, n_r, j_het, fr_n, fr_n_tmp
              n_rain(i,j,k) = 0.0d0
            END IF
...
!              WRITE (*,'(a,3(i3,x),f8.3,4(x,es10.3))') 'SEIFERT_PP RAIN_FREEZE_GAMLOOK: Qgraupel < 0.0 corrected, ', &
!                   i,j,k, T_a, q_r, j_het, fr_q_g, fr_q_tmp
              q_graupel(i,j,k) = 0.0d0
            END IF
            IF (n_graupel(i,j,k) < 0.0) THEN
!              WRITE (*,'(a,3(i3,x),f8.3,4(x,es10.3))') 'SEIFERT_PP RAIN_FREEZE_GAMLOOK: Ngraupel < 0.0 corrected, ', &
!                   i,j,k, T_a, n_r, j_het, fr_n_g, fr_n_tmp
              n_graupel(i,j,k) = 0.0d0
            END IF
            IF (q_hail(i,j,k) < 0.0) THEN
!              WRITE (*,'(a,3(i3,x),f8.3,4(x,es10.3))') 'SEIFERT_PP RAIN_FREEZE_GAMLOOK: Qhail < 0.0 corrected, ', &
!                   i,j,k, T_a, q_r, j_het, fr_q_h, fr_q_tmp
              q_hail(i,j,k) = 0.0d0
            END IF
            IF (n_hail(i,j,k) < 0.0) THEN
!              WRITE (*,'(a,3(i3,x),f8.3,4(x,es10.3))') 'SEIFERT_PP RAIN_FREEZE_GAMLOOK: Nhail < 0.0 corrected, ', &
!                   i,j,k, T_a, n_r, j_het, fr_n_h, fr_n_tmp
              n_hail(i,j,k) = 0.0d0
            END IF
...
!              WRITE (*,*) 'SEIFERT RAIN_FREEZE: Nhail < 0.0, ', i,j,k, T_a, n_hail(i,j,k), j_het, fr_n, fr_n_i, fr_n_g, fr_n_h, fr_n_tmp
  ...
  SUBROUTINE rain_freeze_old ()
    ...
    ALLOCATE(rate_q(0:dim_ix,1:dim_iy,1:dim_iz), &
         rate_n(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)  
...
! UB_20081111:           D_T  = K_T/(cp * rho(i,j,k))
! UB_20081125:            D_T  = K_T / (cp * rho(i,j,k))
! UB_20081125:            fh_q = D_T/D_v * fv_q  ! <-- gives wrong values!
! UB_20081125: After formulas of Rasmussen and Heymsfield (1987) the ratio fh_q / fv_q is approx. 1.05
!              for a wide temperature- and pressure range:
...
! ub>> setzte melt_n so, dass x_s beim Schmelzvorgang erhalten bleibt:
...
#ifdef SAVE_CONVERSIONRATES
            IF (speichere_dqdt) THEN
              dqdt(i,j,k,30) = dqdt(i,j,k,30) + melt_q
            END IF
#endif
          ENDIF
        ENDDO
...
#ifdef SAVE_CONVERSIONRATES
            IF (speichere_dqdt) THEN
              dqdt(i,j,k,29) = dqdt(i,j,k,29) + melt_q
            END IF
#endif
  ...
  SUBROUTINE rain_evaporation_uli ()
    ...
    ALLOCATE(rate_n(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
    ALLOCATE(rate_q(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
...
!          IF(s_sw < 0.0 .AND. q_r > q_krit .AND. q_cloud(i,j,k) < q_krit)THEN
          IF(s_sw < 0.0 .AND. q_r > 0.0d0 .AND. q_cloud(i,j,k) < q_krit)THEN
            ...
            v_r = rain%a_vel * x_r**rain%b_vel * &
                 rrho_04(i,j,k)**(rhovelra+rhovelrb*MIN(D_r,rhovelrdknick))
...
!              eva_n = f_n * eva / x_r
  ...
  SUBROUTINE rain_evaporation ()
    ...
    IF (firstcall.NE.1) THEN
! UB_20100118>> 
      IF (.NOT.use_mu_Dm_rain_evap .AND. (cloud_typ > 1 .AND. use_exponentialdsd_rain_evap)) THEN
        ! In case of using exponential DSD for evaporation:
        rai = rain
        rai%nu = -2.0 / 3.0
        rai%mu = 1.0 / 3.0
        a_q = vent_coeff_a(rai,1)
...
!                  & - 0.4856703981E-05 * mue**4
              ...
              f_q  = rain%a_ven                                          &
              &    + rain%b_ven * N_sc**n_f                              &
              &                 * (aa/nu_l*rrho_04(i,j,k)**(rhovelra+rhovelrb*MIN(D_m,rhovelrdknick)))**m_f          &
              &    * gfak / SQRT(lam)                                    &
              &    * (1.0                                                &
...
!!$              x_r = MAX(1e3/2e7*q_r,MIN(x_r,1e4/2e5*q_r))
              ...
              ELSEIF (use_exponentialdsd_rain_evap) THEN
                x_r = MIN(MAX(q_r/(n_r+eps),rain%x_min),rain%x_max)
                n0r = n_r * (pi*rho_w/x_r)**(1./3.)          !..n0
                n0r = MIN(MAX(6000.0d+03,n0r),20000.0d+03)
                n_r = n0r * (pi*rho_w*n0r/q_r)**(-0.25)     !..Nur q_r  (n_0 = const)
                x_r = q_r / (n_r+eps)
              ELSE
                x_r = MIN(MAX(q_r/(n_r+eps),rain%x_min),rain%x_max)
                x_r = MAX(1e3/2e7*q_r,MIN(x_r,1e4/2e5*q_r))
...
! <UB_20080403
  ...
  ! des wet growth Durchmessers fuer eine bessere Vektorisierung.
  !
  ! - Initialisierung: Einlesen der Lookup-table aus einer Textdatei.
  !   Diese Subroutine muss von der Interface-Routine des 2-M-Schemas 
  !   aufgerufen werden.
...
!!!    The grid vectors for p, qw and qi have to be equidistant.
    ...
    INQUIRE(file=TRIM(dateiname)//'.bin', exist=equifile_exists)

    IF (.NOT.equifile_exists) THEN

      !.. The binary storage file for the equidistant table does
      !   not exist. Therefore, read the original ASCII file
      !   for the non-equidistant table, interpolate the equidistant table
      !   and write it to a storage file.
...
!!!    equidistant Dmin-lookuptable by linear oversampling:
      ...
      ! write the equidistant table to a binary file:
      OPEN(unitnr, file=TRIM(dateiname)//'.bin', status='new', form='unformatted', iostat=error)
      IF (error /= 0) THEN
        WRITE (*,*) 'dmin_wg_gr_ltab_equi: binary lookup-table file ' &
             // TRIM(dateiname)//'.bin' // ' could not be created!'
        STOP
      END IF

      WRITE (unitnr) ltab%n1, ltab%n2, ltab%n3, ltab%n4
      WRITE (unitnr) ltab%x1, ltab%x2, ltab%x3, ltab%x4
      WRITE (unitnr) ltab%dx1, ltab%dx2, ltab%dx3, ltab%dx4
      WRITE (unitnr) ltab%odx1, ltab%odx2, ltab%odx3, ltab%odx4
      WRITE (unitnr) ltab%ltable

      CLOSE (unitnr)

    ELSE   !.. binary file for equidistant table exists

      !.. read the binary file:
      OPEN(unitnr, file=TRIM(dateiname)//'.bin', status='old', form='unformatted', iostat=error)
      IF (error /= 0) THEN
        WRITE (*,*) 'dmin_wg_gr_ltab_equi: binary lookup-table file ' &
             // TRIM(dateiname)//'.bin' // ' could not be opened!'
        STOP
      END IF

      READ (unitnr, iostat=error) ltab%n1, ltab%n2, ltab%n3, ltab%n4
      ...
      READ (unitnr, iostat=error) ltab%x1, ltab%x2, ltab%x3, ltab%x4
      READ (unitnr, iostat=error) ltab%dx1, ltab%dx2, ltab%dx3, ltab%dx4
      READ (unitnr, iostat=error) ltab%odx1, ltab%odx2, ltab%odx3, ltab%odx4
      READ (unitnr, iostat=error) ltab%ltable
...
! These are nowwhere allocated and used except in saturation_adjust_ice()
!   which is in turn not used anywhere!
...
CONTAINS 
  ...
  SUBROUTINE clouds ()
    ...
    ! rho .......... Luftdichte im Grundzustand                                  *
    ...
    USE globale_variablen, ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, dt, &
    ...
    USE konstanten
    USE parallele_umgebung, ONLY: abortparallel,isIO
    USE wolken_konstanten, ONLY: satad_nach_mikrophysik, &
                                 use_rain_freeze_uli, use_ice_graupel_conv_uli
#ifdef COSMOART
    USE art_aci,       ONLY: activ_nenes, icenuc_nenes
    USE wolken_driver, ONLY: calc_i,calc_j,calc_k
   USE data_cosmo_art, ONLY : &
    ...
    REAL             :: wmax, qvmax,qcmax,qrmax,qimax,qsmax,qgmax,precmax,ncmax
...
! Dichteverhaeltnis als Basis fuer die Fallgeschwindigkeitskorr.
! fuer Regen, Eis, Schnee, Graupel und Hagel:
!CDIR COLLAPSE
    rrho_04(0:loc_ix,1:loc_iy,1:loc_iz) = rho0  / rho(0:loc_ix,1:loc_iy,1:loc_iz)

! Fallgeschwindigkeitskorrektur fuer Wolkentropfen nach Stokes-Formel
! (Korrektur mit dem Verh. der dyn. Zaehigkeiten)
    DO k=1, loc_iz
!CDIR COLLAPSE
      DO j=1, loc_iy
        DO i=0, loc_ix
          rrho_c(i,j,k)  = (dyn_visc_air(293.16d0)/dyn_visc_air(T(i,j,k)))**rhovelcb
        END DO
      END DO
    END DO
...
#ifndef FOR_LM
    ! FOR THE COSMO-MODEL, THIS HAS BEEN ALREADY DONE AT THE BEGINNING OF THE INTERFACE!
    ! Hydrometeore auf Werte >= 0.0 und in ihrer mittl. Masse beschraenken:
    q = MAX(q, 0.0d0)
    q_cloud = MAX(q_cloud, 0.0d0)
    q_rain  = MAX(q_rain, 0.0d0)
    IF (ice_typ > 0) THEN
      q_ice     = MAX(q_ice, 0.0d0)
      q_snow    = MAX(q_snow, 0.0d0)
      q_graupel = MAX(q_graupel, 0.0d0)
    END IF
    IF (ice_typ > 1) THEN
      q_hail = MAX(q_hail, 0.0d0)
    END IF
    ...
    ! clean up the n_cloud leftovers when q_cloud is 0:
!CDIR COLLAPSE
    WHERE (q_cloud  == 0.0d0) n_cloud   = 0.0d0

    !ncmax = global_maxval(n_cloud)
    !IF (isIO()) WRITE (*,'(a,2e11.4)') 'CLOUDS 0: max ncloud = ',ncmax*1e-6

    IF (isdebug) WRITE (*,*) 'CLOUDS: cloud_nucleation'
    IF (cloud_typ > 1 ) THEN

#ifdef COSMOART
!set diagnenes to zero
!diagnenes=0.0 ! keep diagnenes for calculation of hourly mean
activ_nenes = .TRUE.
    IF (activ_nenes) THEN
     WRITE(*,*) 'Activ Nenes'

        CALL nucleation_cosmoart

    ELSE
#endif
      IF (nuc_c_typ .eq. 0) THEN
        IF (isdebug) WRITE (*,*) '  ... force constant cloud droplet number conc.'
        WHERE (q_cloud > 0.0d0) n_cloud = qnc_const
...
! UB_20090227<<
    ...
    !.. Limit mean mass of cloud droplets to [xmin,xmax]:
    CALL limit_only_dp( q_cloud  (0:loc_ix, 1:loc_iy, 1:loc_iz), &
                        n_cloud  (0:loc_ix, 1:loc_iy, 1:loc_iz), &
                        cloud,    0,loc_ix, 1,loc_iy, 1,loc_iz )
...
#ifdef COSMOART
icenuc_nenes = .TRUE.
    IF (icenuc_nenes) THEN
     WRITE(*,*) 'Ice Nucleation Nenes'

       CALL ice_nucleation_cosmoart
...
! UB_20090227<<
    ...
    ! Hard upper limit for ice number conc. to 500 per liter in any case:
    n_ice = MIN(n_ice, 500d3)
...
#ifdef COSMOART
!MB_2010
!Output for means of cloud properties with standard COSMO-Output procedure (GRIB, NetCdf).
      DO k = 1, loc_iz
        DO j = 1, loc_iy
          DO i = 0, loc_ix
 
          ii=calc_i(i) ! get domain indices
          jj=calc_j(i)
          kk=calc_k(i)


            IF (q_cloud(i,j,k) > 1.d-10) THEN   ! maybe use greater value for "real" clouds
             diagnenes(ii,jj,kk,15)= diagnenes(ii,jj,kk,15)+REAL(q_cloud(i,j,k),ireals)
             diagnenes(ii,jj,kk,16)= diagnenes(ii,jj,kk,16)+REAL(n_cloud(i,j,k),ireals)
             diagnenes(ii,jj,kk,17)= diagnenes(ii,jj,kk,17)+1.0_ireals
            ENDIF

            IF (q_rain(i,j,k) > 1.d-15) THEN   
             diagnenes(ii,jj,kk,18)= diagnenes(ii,jj,kk,18)+REAL(q_rain(i,j,k),ireals)
             diagnenes(ii,jj,kk,19)= diagnenes(ii,jj,kk,19)+1.0_ireals
            ENDIF            

            IF (q_ice(i,j,k) > 1.d-15) THEN   
             diagnenes(ii,jj,kk,20)= diagnenes(ii,jj,kk,20)+REAL(q_ice(i,j,k),ireals)
             diagnenes(ii,jj,kk,21)= diagnenes(ii,jj,kk,21)+REAL(n_ice(i,j,k),ireals)
             diagnenes(ii,jj,kk,22)= diagnenes(ii,jj,kk,22)+1.0_ireals
            ENDIF

            IF (q_snow(i,j,k)+q_graupel(i,j,k)+q_hail(i,j,k) > 1d-15) THEN   
             diagnenes(ii,jj,kk,23)= diagnenes(ii,jj,kk,23)+REAL(q_snow(i,j,k)+q_graupel(i,j,k)+q_hail(i,j,k),ireals)
             diagnenes(ii,jj,kk,24)= diagnenes(ii,jj,kk,24)+1.0_ireals
            ENDIF
...
!MB_2010
        ...
        DO j = 1, loc_iy
          DO i = 0, loc_ix
            ...
            IF (L_c > 0.d0 .AND. L_r > 0.d0) THEN
              ...
              ac = C_ac * L_c * L_r**0.875 * rrho_c(i,j,k) * dt
        ...
        DO j = 1, loc_iy
          DO i = 0, loc_ix
            ...
            IF (ts(i, j, k) > T_f) THEN ! Saettigung ueber Wasser
              ...
              p(i, j, k) = p(i, j, k) - r * rho(i, j, k) * lrcv * dq_d
        ...
        DO j = 1, loc_iy
          DO i = 0, loc_ix
            IF (ts(i, j, k) < T_3) THEN ! Saettigung ueber Eis
              ...
              p(i, j, k) = p(i, j, k) - r * rho(i, j, k) *   a3 * rcv * dq_d
  ...
  SUBROUTINE alloc_wolken ()
    ...
    USE globale_variablen,  ONLY: q_cloud, q_rain, q_ice, q_snow, q_graupel,  &
         ...
         &                        q_hail, n_hail,                  &
         &                        loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, &
    ...
    IF (firstcall /= 1) THEN
      ...
      IF(isIO()) THEN
        WRITE (*,*) "CLOUD alloc_wolken: wolke_typ = ", wolke_typ
        WRITE (*,*) "                    ice_typ   = ", ice_typ
        WRITE (*,*) "                    nuc_i_typ = ", nuc_i_typ
        WRITE (*,*) "                    nuc_c_typ = ", nuc_c_typ
        WRITE (*,*) "                    cloud_typ = ", cloud_typ
      END IF

      ALLOCATE (n_cloud(0:dim_ix, 1:dim_iy, 1:dim_iz), &
           q_cloud(0:dim_ix, 1:dim_iy, 1:dim_iz), &
           n_rain(0:dim_ix, 1:dim_iy, 1:dim_iz),  &
           q_rain(0:dim_ix, 1:dim_iy, 1:dim_iz),  &
           n_ice(0:dim_ix, 1:dim_iy, 1:dim_iz),   &
           q_ice(0:dim_ix, 1:dim_iy, 1:dim_iz),   &             
           n_snow(0:dim_ix, 1:dim_iy, 1:dim_iz),   &
           q_snow(0:dim_ix, 1:dim_iy, 1:dim_iz),   &             
           n_hail(0:dim_ix, 1:dim_iy, 1:dim_iz),   &
           q_hail(0:dim_ix, 1:dim_iy, 1:dim_iz),   &
           n_graupel(0:dim_ix, 1:dim_iy, 1:dim_iz), &
           q_graupel(0:dim_ix, 1:dim_iy, 1:dim_iz), &
           STAT = stat_var)
      IF (stat_var /= 0) THEN
        CALL abortparallel ("Fehler bei Allokierung Hydrometeore ", stat_var)
      END IF
            
#ifdef SAVE_CONVERSIONRATES
      IF (speichere_dqdt) THEN
        ALLOCATE (dqdt(0:dim_ix, 1:dim_iy, 1:dim_iz, nmicrorates), STAT = stat_var)
        IF (stat_var /= 0) THEN
...
#ifdef SAVE_PRECIPSTATISTICS
      IF (speichere_precipstat) THEN
        ALLOCATE(cond_neu_sb(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                 evap_neu_sb(0:dim_ix, 1:dim_iy, 1:dim_iz), STAT = stat_var)
        IF (stat_var /= 0) THEN
      ...
      ALLOCATE(rrho_04(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
      ALLOCATE(rrho_c (0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
  ...
  SUBROUTINE autoconversionSB ()
    ...
    ALLOCATE(rate_q(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
    ALLOCATE(rate_nr(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
    ALLOCATE(rate_nc(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
      ...
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          ...
          IF (q_c > eps) THEN
            ...
            IF (q_c > 1.0d-6) THEN
              tau = MIN(MAX(1.0-q_c/(q_c+q_r+eps),eps),1.0d0-1.0d-1)      
   ...
   DO k = 1, loc_iz
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          ...
          jj=calc_j(i)
          kk=calc_k(i)
          diagnenes(ii,jj,kk,25)= diagnenes(ii,jj,kk,25)+REAL(rate_q(i,j,k),ireals)
          diagnenes(ii,jj,kk,26)= diagnenes(ii,jj,kk,26)+1.0_ireals
        END DO
      END DO
    END DO
#endif
  ...
  SUBROUTINE accretionSB ()
    ...
    INTEGER          :: i,j,k
    DOUBLE PRECISION :: ac
    DOUBLE PRECISION :: L_c, L_r, L_g, tau, phi, n_c, x_c, x_r, D_r
      ...
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          ...
          IF (L_c > 0.0d0.AND.L_r > 0.0d0) THEN
            ...
            x_r = MIN(MAX(L_r/(n_rain(i,j,k)+eps),rain%x_min),rain%x_max)
            D_r = rain%a_geo * x_r**rain%b_geo
            ...
            phi = (tau/(tau+k_1))**4
            ac  = k_r *  L_c * L_r * phi  * &
                 rrho_04(i,j,k)**(rhovelra+rhovelrb*MIN(D_r,rhovelrdknick)) * dt
  ...
  SUBROUTINE rain_selfcollectionSB ()
    ...
    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, n_rain, q_rain, dt
    USE konstanten,         ONLY: pi 
    USE parallele_umgebung, ONLY: isIO
  ...
  SUBROUTINE autoconversionKB ()
    ...
    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, q_cloud, q_rain, n_rain, dt
    USE konstanten,         ONLY: pi
    USE parallele_umgebung, ONLY: isIO,abortparallel
    USE globale_variablen,  ONLY: dqdt, speichere_dqdt
  ...
  SUBROUTINE vapor_deposition_growth(dt_local,i_local)
    ...
    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, T, p, q, rho, &
    ...
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:,:), SAVE :: rime_q_gc, rime_q_gr
    ...
    INTEGER                     :: i,j,k
    INTEGER, SAVE               :: firstcall
    DOUBLE PRECISION, SAVE      :: gr_sn_fakt_dep, gr_sn_fakt_rim

    ALLOCATE(s_si(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(s_sw(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(g_i(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(g_w(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(dep_ice(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(dep_snow(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(dep_cloud(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(dep_graupel(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(dep_graupel_n(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(dep_hail(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(dep_hail_n(0:dim_ix,1:dim_iy,1:dim_iz))
    ...
    IF (firstcall.NE.1) THEN
      IF (graupel_to_snow) THEN
        !.. Shape factors for graupel to snow conversion:
        gr_sn_fakt_dep = fakt_gr_sn_dep(graupel)
        gr_sn_fakt_rim = fakt_gr_sn_rim(graupel)
      END IF
      IF(isIO() .AND. isdebug)THEN
        ...
        IF (graupel_to_snow) THEN
          WRITE (6,'(A,D10.3)') "  Tuning-Faktor 1 fuer graupel to snow  gr_sn_conv_tune_1 = ",gr_sn_conv_tune_1
          WRITE (6,'(A,D10.3)') "  Tuning-Faktor 2 fuer graupel to snow  gr_sn_conv_tune_2 = ",gr_sn_conv_tune_2
          WRITE (6,'(A,D10.3)') "  Shape-Faktor (dep) fuer graupel to snow  gr_sn_fakt_dep = ",gr_sn_fakt_dep
          WRITE (6,'(A,D10.3)') "  Shape-Faktor (rim) fuer graupel to snow  gr_sn_fakt_rim = ",gr_sn_fakt_rim
          WRITE (6,'(A,D10.3)') "  Max. D_g fuer graupel to snow                D_g_max_gs = ",D_g_max_gs
          WRITE (6,'(A,D10.3)') "  Max. D_s des erz. Schnees f. graupel to snow  D_conv_gs = ",D_conv_gs
        END IF
      END IF
      firstcall = 1
    ENDIF

    IF (i_local == 1 .AND. graupel_to_snow) THEN
      IF (ALLOCATED(rime_q_gc)) DEALLOCATE(rime_q_gc)
      ALLOCATE(rime_q_gc(0:dim_ix,1:dim_iy,1:dim_iz))
      IF (ALLOCATED(rime_q_gr)) DEALLOCATE(rime_q_gr)
      ALLOCATE(rime_q_gr(0:dim_ix,1:dim_iy,1:dim_iz))
      ! Store the graupel riming rates (these are pre_multiplied with dt) for later use
      ! in "graupel to snow conversion" below
      CALL graupel_cloud_riming_rate(rime_q_gc)
      CALL graupel_rain_riming_rate (rime_q_gr)
    END IF
...
! UB_20090316:          D_vtp = D_v
    ...
    !q       = q       - dep_cloud
    !T       = T       + dep_cloud * L_ed / cp / rho(i,j,k) 

! UB    x_conv = (250e-6/snow%a_geo)**(1./snow%b_geo)
    x_conv = (D_conv_gs/snow%a_geo)**(1./snow%b_geo)         ! <UB
...
!!! das urspruengliche Verhaeltnis der Depositionsfluesse massgeblich sein soll.
            ...
            ELSE
              ...
              available_d = s_si(i,j,k) * e_es(T(i,j,k)) / (R_d * T(i,j,k))
            ...
            IF (graupel_to_snow .AND. dep_graupel(i,j,k) > 0.0 .AND. &
                 dep_graupel(i,j,k) > 0.0 .AND. &
                 D_g < D_g_max_gs) THEN         ! <UB
              !..Graupel to snow conversion
! UB>> 20090930: Neue Parametrisierung der graupel to snow conversion:
              conv_q = gr_sn_conv_tune_1 * MAX(gr_sn_fakt_dep*dep_graupel(i,j,k) - &
                   gr_sn_conv_tune_2 * gr_sn_fakt_rim*(rime_q_gc(i,j,k)+rime_q_gr(i,j,k)), 0.0d0)
              conv_q           = MIN(conv_q, q_g + dep_graupel(i,j,k))
              q_snow(i,j,k)    = q_snow(i,j,k)    + conv_q
...
!!$ UB_20120914            f_v  = max(f_v,1.d0) !unnoetig??
            f_v  = MAX(f_v,a_f/cloud%a_ven)     !!$ UB_20120914
        ...
        DO j = 1, loc_iy
          DO i = 0, loc_ix
            ...
            ELSE  
              ...
              v_i = ice%a_vel * EXP(ice%b_vel*LOG(x_i)) * &
                   rrho_04(i,j,k)**(rhovelia+rhovelib*MIN(D_i,rhovelidknick))    !..mittlere Sedimentationsgeschw.

              N_re = v_i * d_i / nu_l                              !..mittlere Reynoldszahl
              ! N_re**m_f durch EXP(m_f*LOG(N_re)) ersetzen, da N_re == 0 nicht mehr vorkommen kann:
              f_v  = a_f + b_f * f_v_fakt * EXP(m_f*LOG(N_re))     !..mittlerer Vent.Koeff.
!!$ UB_20120914              f_v  = MAX(f_v,1.0d0) !unnoetig??
              f_v  = MAX(f_v,a_f/ice%a_ven)     !!$ UB_20120914
        ...
        DO j = 1, loc_iy
          DO i = 0, loc_ix
            ...
            ELSE
              ...
              v_g = graupel%a_vel * EXP(graupel%b_vel*LOG(x_g)) * &
                   rrho_04(i,j,k)**(rhovelga+rhovelgb*MIN(D_g,rhovelgdknick))  !..mittlere Sedimentationsgeschw.
              ...
              ! ub<<
!!$ UB_20120914             f_v  = max(f_v,1.d0) !unnoetig??
!!$ UB_20120914             f_n  = max(f_n,1.d0) !unnoetig??
              f_v  = MAX(f_v,a_f/graupel%a_ven)     !!$ UB_20120914
              f_n  = MAX(f_n,a_n/graupel%a_ven)     !!$ UB_20120914
        ...
        DO j = 1, loc_iy
          DO i = 0, loc_ix
            ...
            ELSE
              ...
              v_h = hail%a_vel * EXP(hail%b_vel*LOG(x_h)) * &
                   rrho_04(i,j,k)**(rhovelha+rhovelhb*MIN(D_h,rhovelhdknick))  !..mittlere Sedimentationsgeschw.
...
!!$ UB_20120914             f_v  = max(f_v,1.d0) !unnoetig??
!!$ UB_20120914             f_n  = max(f_n,1.d0) !unnoetig??
              f_v  = MAX(f_v,a_f/hail%a_ven)     !!$ UB_20120914
              f_n  = MAX(f_n,a_n/hail%a_ven)     !!$ UB_20120914
        ...
        DO j = 1, loc_iy
          DO i = 0, loc_ix
            ...
            ELSE
              ...
              v_s = snow%a_vel * EXP(snow%b_vel*LOG(x_s)) * &
                   rrho_04(i,j,k)**(rhovelsa+rhovelsb*MIN(D_s,rhovelsdknick))   !..mittlere Sedimentationsgeschw.
              ...
              f_v  = a_f + b_f * f_v_fakt * EXP(m_f*LOG(N_re))        !..mittlerer Vent.Koeff.
!!$ UB_20120914              f_v  = max(f_v,1.d0) !unnoetig??
              f_v  = MAX(f_v,a_f/snow%a_ven)     !!$ UB_20120914
  ...
  SUBROUTINE vapor_dep_simple(dt_local,i_local)
    ...
    DOUBLE PRECISION            :: dt_local
    DOUBLE PRECISION            :: a_dl,a_ld,D_vtp
    DOUBLE PRECISION            :: q_g,n_g,x_g,D_g,q_s,n_s,x_s,conv_n,x_conv
    ! ub>> 
    DOUBLE PRECISION            :: necessary_d,available_d,weight,depitmp,depstmp,depgtmp,dephtmp
    ! ub<<
    DOUBLE PRECISION, PARAMETER :: eps  = 1.d-20
    ...
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:,:) :: s_si,g_i
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:,:) :: s_sw,g_w,conv_q  ! <UB
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:,:) :: dep_ice
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:,:) :: dep_snow
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:,:) :: dep_cloud
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:,:) :: dep_graupel, dep_graupel_n
    ...
    ALLOCATE(dep_hail(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(dep_hail_n(0:dim_ix,1:dim_iy,1:dim_iz))
    ALLOCATE(conv_q(0:dim_ix,1:dim_iy,1:dim_iz))           ! <UB
...
!!$              END IF
            ...
            IF (graupel_to_snow .AND. dep_graupel(i,j,k) > 0.0 .AND. &
              ...
              !..Graupel to snow conversion
! UB>> 20090930 Neue Parametrisierung der graupel to snow conversion:
              conv_q(i,j,k) = gr_sn_conv_tune_1 * MAX(gr_sn_fakt_dep*dep_graupel(i,j,k) - &
                   gr_sn_conv_tune_2 * gr_sn_fakt_rim*(rime_q_gc(i,j,k)+rime_q_gr(i,j,k)), 0.0d0)
              conv_q(i,j,k) = MIN(conv_q(i,j,k), q_g + dep_graupel(i,j,k))
              q_snow(i,j,k)    = q_snow(i,j,k)    + conv_q(i,j,k)
              q_graupel(i,j,k) = q_graupel(i,j,k) - conv_q(i,j,k) + dep_graupel(i,j,k)
              n_snow(i,j,k)    = n_snow(i,j,k)    + conv_q(i,j,k) / MIN(x_g,x_conv)
              n_graupel(i,j,k) = n_graupel(i,j,k) - conv_q(i,j,k) / x_g      
            else
              q_graupel(i,j,k) = q_graupel(i,j,k) + dep_graupel(i,j,k)
              conv_q(i,j,k) = 0.0
            endif
        ...
        DO j = 1, loc_iy
          DO i = 0, loc_ix
            ...
            IF (dep_ice(i,j,k) + dep_graupel(i,j,k) + dep_snow(i,j,k) + dep_hail(i,j,k) >= 0.0) THEN
              ...
              dqdt(i,j,k,6) = dqdt(i,j,k,6) + dep_graupel(i,j,k)
              dqdt(i,j,k,55) = dqdt(i,j,k,55) + dep_hail(i,j,k)
              dqdt(i,j,k,7) = dqdt(i,j,k,7) + conv_q(i,j,k)
            ELSE
              dqdt(i,j,k,56) = dqdt(i,j,k,56) - dep_ice(i,j,k)
              dqdt(i,j,k,32) = dqdt(i,j,k,32) - dep_snow(i,j,k)
              dqdt(i,j,k,33) = dqdt(i,j,k,33) - dep_graupel(i,j,k)
...
!!$ UB_20120914              f_v  = max(f_v,1.d0) !unnoetig??
  ...
  ! ub> fuer graupel to snow conversion:
  DOUBLE PRECISION FUNCTION fakt_gr_sn_dep(p)
    IMPLICIT NONE
    TYPE(PARTICLE), INTENT(in)          :: p
    DOUBLE PRECISION                    :: mom
    
    mom = 1.5 * p%b_geo + 0.5 * p%b_vel
    
    fakt_gr_sn_dep = gfct((mom+p%nu)/p%mu) * gfct((p%nu+2.0)/p%mu)   &
         / ( gfct((mom+p%nu+1.0)/p%mu) * gfct((p%nu+1.0)/p%mu) )
    
    RETURN
  END FUNCTION fakt_gr_sn_dep
  
  ! ub> fuer graupel to snow conversion:
  DOUBLE PRECISION FUNCTION fakt_gr_sn_rim(p)
    IMPLICIT NONE
    TYPE(PARTICLE), INTENT(in)          :: p
    DOUBLE PRECISION                    :: mom
    
    mom = 2.0 * p%b_geo + p%b_vel
    
    fakt_gr_sn_rim = gfct((mom+p%nu)/p%mu) * gfct((p%nu+2.0)/p%mu)   &
         / ( gfct((mom+p%nu+1.0)/p%mu) * gfct((p%nu+1.0)/p%mu) )
    
    RETURN
  END FUNCTION fakt_gr_sn_rim
  ...
  SUBROUTINE cloud_nucleation()
    ...
    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, &
                                  T, p, q, rho, q_cloud, n_cloud, &
         &                        w, dt,S_w, dSwdz, zml_k,    &
         &                        dqdt, speichere_dqdt, cond_neu_sb,     &
         &                        evap_neu_sb, speichere_precipstat
    USE konstanten,         ONLY: r,cv,cp
    USE parallele_umgebung, ONLY: isIO,abortparallel
    ...
    ! Locale Variablen 
    DOUBLE PRECISION            :: T_a             !..Absolute Temperatur
    DOUBLE PRECISION            :: q_d,x_d,e_d,n_c,rho_a
    ...
    !ALLOCATE(s_sw(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
    !ALLOCATE(dSdz(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
...
!<AS20080426
  ...
  SUBROUTINE cloud_nucleation_SK()
    ...
    DOUBLE PRECISION   :: q_d,x_d,e_d,n_c,q_c,rho_a
    ...
    !..parameter for exponential decrease of N_ccn with height:
    !  1) up to this height (m) constant unchanged value:
! UB_20091127: now a namelist parameter!    z0_nccn = 4000.0d0
    z0_nccn = z_ncn_segal
    !  2)  height interval at which N_ccn decreses by factor 1/e above z0_nccn:
! UB_20091127: now a namelist parameter!        z1e_nccn = 2000.0d0
    z1e_nccn = z1oe_ncn_segal
...
!          IF (w_cb(i,j,k) <= 0.00d0 .or. T(i,j,k) < 263.0d0 ) CYCLE
...
! beeing larger than N_cn, limit NCCN by N_cn:
!          tab_Ndrop_i = ip_ndrop_ncn(tab_Ndrop,tab_Ncn,N_cn)
          tab_Ndrop_i = MIN(ip_ndrop_ncn(tab_Ndrop,tab_Ncn,N_cn), N_cn)
! HN_20080401<
...
#ifndef FOR_LM
  WRITE(*,'(a,2x,f10.3)') 'maximum w at cloud base:', MAXVAL(w_cb)
  WRITE(*,'(a,2x,e12.5)') 'maximum nucleation (1/m**3):', nucn_max 
#endif
  ...
  CONTAINS
    ...
    SUBROUTINE lookuptable(tab_ndrop,i_lsigs,i_R2)
      ...
      ndrop3_23 =  (/  45.6d06,  91.8d06, 171.5d06, 314.9d06, 559.0d06, 932.8d06,1501.6d06,1501.6d06/)
  ...
  SUBROUTINE clnuc_sk_4d()
    ...
    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, T, p, q, q_cloud, n_cloud, &
         &                        dt, w_cb, dqdt, zml_k,       &
                                  speichere_dqdt, cond_neu_sb,           &
                                  &evap_neu_sb, speichere_precipstat
    USE konstanten,         ONLY: r,cv,cp
...
!          IF (w_cb(i,j,k) > 0.00d0 .AND. T(i,j,k) >= 263.0d0 ) THEN
...
#ifdef COSMOART
  SUBROUTINE nucleation_cosmoart
    !*******************************************************************************
    ! Calculation of cloud drop nucleation with parameterisations of Nenes ... 
    ! references need to be added!!
    !*******************************************************************************

    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, T, p, q, q_cloud, n_cloud, &
                                  dt, w_cb, dqdt, zml_k,n_rain,&
                                  speichere_dqdt, cond_neu_sb,calc_i,calc_j,calc_k,  &
                                  evap_neu_sb, speichere_precipstat, cloudact
    USE konstanten,         ONLY: r,cv,cp
    USE parallele_umgebung, ONLY: isIO,abortparallel
    USE art_aci,            ONLY: call_activation
    ...
    DOUBLE PRECISION         :: n_c,q_c
    DOUBLE PRECISION         :: nuc_n, nuc_q, nucn_max,Ntotal, Npact
    DOUBLE PRECISION         :: wcb, deltat,dbledummy 
    DOUBLE PRECISION         :: r2_loc, lsigs_loc, ncn_loc, wcb_loc
    INTEGER                  :: i, j, k, ii,jj,kk,idummy
    INTEGER, SAVE            :: firstcall

   ! select timelevel and timestep for calculations                          
     deltat = dt ! must be consisten to dt in modelconfig!!! and dt used in call_activation
     dbledummy=99.0
     idummy=99
    IF (firstcall .NE. 1) THEN 
      CALL call_activation(1,1,idummy,idummy,idummy,dbledummy,dbledummy,dbledummy) ! Calculate data for updraft PDF   
      firstcall = 1
    ENDIF
...
!            that would come from the dynamical core
            ...
            nuc_q = 0.0d0
            nuc_n = 0.d0
            n_c   = n_cloud(i,j,k)+n_rain(i,j,k)
            q_c   = q_cloud(i,j,k)
            wcb   = w_cb(i,j,k)
            Ntotal = 0.0d0

 CALL call_activation(2,cloudact(i,j,k),ii,jj,kk,n_c,Ntotal,Npact)

            nuc_n = deltat * Npact ! calculate nucleation 

            IF(nuc_n+n_cloud(i,j,k) > Ntotal) THEN
!            PRINT*, 'nuc_n to big:', nuc_n, n_cloud(i,j,k), Ntotal
               nuc_n=Ntotal-n_cloud(i,j,k)
            ENDIF
...
!          WRITE(*,*) 'Nucleation:',ii,jj,kk,' wcb=',wcb,'  nuc_n=',nuc_n, &
!                     ' ncloud=',n_cloud(i,j,k),' Npact=',Npact,           &
!                     ' cloudact(i,j,k)=',cloudact(i,j,k)
  ...
  END SUBROUTINE nucleation_cosmoart
#endif

#ifdef COSMOART 
SUBROUTINE ice_nucleation_cosmoart
!*******************************************************************************
! Calculation of cloud ice nucleation with parameterisations of Nenes ... 
! references need to be added!!
!*******************************************************************************
  ...
  USE art_aci,            ONLY: call_icenucleation

  IMPLICIT NONE

  DOUBLE PRECISION, ALLOCATABLE :: nuc_n(:,:,:), nuc_q(:,:,:)
  DOUBLE PRECISION         :: Nnuci
  INTEGER                  :: i, j, k, ii,jj,kk,typenuci,stat_var
  ...
  IF (stat_var /= 0) THEN
      WRITE (*,*) 'Fehler bei Allokierung in ice_nucleation_cosmoart ', stat_var
      STOP
  END IF
  ...
  nuc_q = 0.0d0
        ...
        DO k = 1, loc_iz
          DO j = 1, loc_iy
            DO i = 0, loc_ix
              IF (T(i,j,k) < T_nuc .AND. s_i(i,j,k) > 0.0  &
                  .AND. ( n_ice(i,j,k)+n_snow(i,j,k) < ni_het_max ) )THEN ! other limit than ni_het_max??

          ii=calc_i(i) ! get domain indices
          jj=calc_j(i)
          kk=calc_k(i)
                IF (q_cloud(i,j,k) > 0.0) THEN
                  ! MB set hom het settings
                  ! immersion freezing at water saturation
                  ! seifert routine dropfreeze additionally??
!                  typenuci = 1
                   typenuci = 2 !probably always true
                ELSE
                  typenuci = 2
                ENDIF

         CALL call_icenucleation(typenuci,ii,jj,kk,Nnuci)

                nuc_n_tmp = MAX( Nnuci - (n_ice(i,j,k)+n_snow(i,j,k)),0.d0)
                nuc_n(i,j,k) = nuc_n(i,j,k) + nuc_n_tmp
...
!         WRITE(*,*) 'ICEnukl:',ii,jj,kk,'  nuc_n(i,j,k)=',nuc_n(i,j,k), &
!                     ' ncice=',n_ice(i,j,k),' Nnuci=',Nnuci
  ...
  END SUBROUTINE ice_nucleation_cosmoart
#endif

!==========================================

  SUBROUTINE clip_and_limit_dp(qx, nx, parti, ims, ime, jms, jme, kms, kme)
    ...
    INTEGER, INTENT(IN)             :: ims, ime, jms, jme, kms, kme
    DOUBLE PRECISION, INTENT(INOUT) :: qx(ims:ime, jms:jme, kms:kme), &
                                       nx(ims:ime, jms:jme, kms:kme)
    TYPE(particle), INTENT(IN)      :: parti
...
!CDIR COLLAPSE
      nx(:,:,k) = MAX(nx(:,:,k), qx(:,:,k) / parti%x_max)
!CDIR COLLAPSE
      nx(:,:,k) = MIN(nx(:,:,k), qx(:,:,k) / parti%x_min)

    END DO

  END SUBROUTINE clip_and_limit_dp

  SUBROUTINE limit_only_dp(qx, nx, parti, ims, ime, jms, jme, kms, kme)
...
!CDIR COLLAPSE
  ...
  END SUBROUTINE limit_only_dp

  SUBROUTINE clip_only_dp(qx, ims, ime, jms, jme, kms, kme)
...
!CDIR COLLAPSE
  ...
  END SUBROUTINE clip_only_dp


#ifdef FOR_LM

  SUBROUTINE clip_and_limit_ireals(qx, nx, parti, ims, ime, jms, jme, kms, kme)

    USE wolken_konstanten, ONLY : particle
    USE data_parameters,   ONLY : iintegers, ireals => dp

    IMPLICIT NONE

    INTEGER(kind=iintegers), INTENT(IN) :: ims, ime, jms, jme, kms, kme
    REAL(kind=ireals), INTENT(INOUT)    :: qx(ims:ime, jms:jme, kms:kme), &
                                           nx(ims:ime, jms:jme, kms:kme)
    TYPE(particle), INTENT(IN)          :: parti
...
!CDIR COLLAPSE
  ...
  END SUBROUTINE clip_and_limit_ireals

  SUBROUTINE limit_only_ireals(qx, nx, parti, ims, ime, jms, jme, kms, kme)
...
!CDIR COLLAPSE
  ...
  END SUBROUTINE limit_only_ireals

  SUBROUTINE clip_only_ireals(qx, ims, ime, jms, jme, kms, kme)
...
!CDIR COLLAPSE
  ...
  END SUBROUTINE clip_only_ireals
