...
! 3.3        2015/09/15 Prabhakar Shrestha
!   Band-aid for initial coupling removed. 
!
...
USE data_tracer     , ONLY :                &
                     molmass_co2,&       ! molar mass of CO2                           (g/mol)
...
REAL(KIND=ireals) ::   ztmp1(ie,je),       &      ! temporary array
                       umass(ie,je), vmass(ie,je)

REAL(kind=ireals), SAVE ::   tic, toc
...
!   cplfreq = NINT ( hincrad * 3600.0_ireals)        !CPS   !FG Why should  this
...
!CPS   IF (llandmask(i,j)) THEN        !land points only
   ...
   IF( srcv(jps_co2fl)%laction ) THEN
     co2fl_s(i,j) = frcv(i,j,jps_co2fl) * 1.e-6_ireals * (molmass_co2 * 0.001) / &
                                             ( rho(i,j,ke) * ( hhl(i,j,ke) - hhl(i,j,ke1) ) )
   ELSE
     co2fl_s(i,j) = 0.
   ENDIF
!MU (18.09.12)
   IF( srcv(jps_fpsn)%laction ) THEN
     psn_tens(i,j) = frcv(i,j,jps_fpsn) * 1.e-6_ireals * (molmass_co2 * 0.001) / &
                                             ( rho(i,j,ke) * ( hhl(i,j,ke) - hhl(i,j,ke1) ) )
   ELSE
     psn_tens(i,j) = 0.
   ENDIF
   ...
   IF( srcv(jps_fplres)%laction ) THEN
     plres_tens(i,j) = frcv(i,j,jps_fplres) * 1.e-6_ireals * (molmass_co2 * 0.001) / &
                                             ( rho(i,j,ke) * ( hhl(i,j,ke) - hhl(i,j,ke1) ) )
   ELSE
     plres_tens(:,:) = 0.
   ENDIF
...
!CPS alb_rad dimension changed for coupling
   ...
   IF( srcv(jps_ir)%laction ) THEN
    t_g(i,j,nnew) = (frcv(i,j,jps_ir) / sigma / (1._ireals - ctalb))**0.25 
   ENDIF
!CPS   t_g(:,:,nx) = t_g(:,:,nnew)   
!CPS   t_s(:,:,nx)   = t_g(:,:,nx)       !CPS ground temperature 
...
!CPS
     IF (cpl_scheme) THEN
        !CPS non-dimensionlized conductance for air
         tcm(i,j) = 1._ireals/(tcm(i,j)*MAX(zvbke,vel_min))   !Estimated based on CLM
         tch(i,j) = 1._ireals/(tch(i,j)*MAX(zvbke,vel_min))   !Estimated based on CLM
#ifdef COUP_OAS_COS
         tcw(i,j) = 1._ireals/(tcw(i,j)*MAX(zvbke,vel_min))   !Estimated based on CLM
#endif
...
!CPS Some fixes for real case simulation
    ...
    ! Latent heat flux inversion: derive new qv_s from clm flux
        IF (tch(i,j) .NE. 0) THEN
          ...
          qv_s(i,j,nx) = -qvsflx(i,j) / ( ztch(i,j)*gr)  + qv(i,j,ke,nx)      
          qv_s(i,j,nnew) = qv_s(i,j,nx)
        ENDIF

! Momentum flux inversion: derive new 6tcm from clm3.5 momentum flux(use either u or v ) !CPS
!
        umass(i,j) = 0.5_ireals *(u(i,j,ke,nx) + u(im1,j,ke,nx))
        vmass(i,j) = 0.5_ireals *(v(i,j,ke,nx) + v(im1,j,ke,nx))
...
! umfl , vmfl from CLM are in mass points and tcm, tch is also in mass point
        IF ( zvbke == 0 .OR. umass(i,j) == 0) THEN         !CPS nonzero denominator
          tcm(i,j) = 0._ireals !CPS stable condition
        ELSE
