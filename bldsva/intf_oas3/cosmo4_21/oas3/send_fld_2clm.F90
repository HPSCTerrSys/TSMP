...
USE data_tracer      , ONLY :   &
                                    tracer, &                 ! tracer concentration                  (kg/kg)
                                    ntracer, &                ! number of tracers
                                    molmass_da, &             ! molar mass of dry air                 (g/mol)
                                    molmass_w, &              ! molar mass of water                   (g/mol)
...
REAL(KIND=ireals)       ::   ztmp1(ie,je),     &! temporary arrays
                             umass(ie,je),     &!
                             vmass(ie,je),     &!
                             winds(ie,je) 
REAL(KIND=ireals)       :: toc
...
!MU (13.09.2012)
          DO iig=1, ntracer
            IF ( jn == 17 .and. ltracer(7,iig) == 1 ) ztmp1(:,:) = tracer(:,:,ke,nnow,iig)
          ENDDO
!MU (13.09.2012)
 ...
 IF ( lpe_cpl ) THEN
   ...
   DO j = jstartpar, jendpar
     ...
     DO i = istartpar, iendpar
       im1 = MAX(i-1,1)                !CPS
       umass(i,j)  = 0.5_ireals*(u(i,j,ke,nnow) + u(im1,j,ke,nnow))
       vmass(i,j)  = 0.5_ireals*(v(i,j,ke,nnow) + v(i,jm1,ke,nnow))
   ...
   IF( ssnd(jps_q)%laction )   CALL oas_cos_snd( jps_q, isec, qv(:,:,ke,nnow), info )

   ! Send only total height
   ztmp1(:,:) = (hhl(:,:,ke) - hsurf(:,:)) * 0.5_ireals   !CPS
...
!MU (13.09.2012)
   ! Need to send CO2 partial pressure for CLM
     ...
     IF (itype_gscp == 4) THEN
       molmass_ma(:,:,ke) = ( 1 - qv(:,:,ke,nnow) - qc(:,:,ke,nnow) - qi(:,:,ke,nnow) &
                                - qr(:,:,ke,nnow) - qs(:,:,ke,nnow) - qg(:,:,ke,nnow) ) * molmass_da &
                            + (   qv(:,:,ke,nnow) + qc(:,:,ke,nnow) + qi(:,:,ke,nnow) &
                                + qr(:,:,ke,nnow) + qs(:,:,ke,nnow) + qg(:,:,ke,nnow) ) * molmass_w
     ELSE
       molmass_ma(:,:,ke) = ( 1 - qv(:,:,ke,nnow) - qc(:,:,ke,nnow) - qi(:,:,ke,nnow) &
                                - qr(:,:,ke,nnow) - qs(:,:,ke,nnow) ) * molmass_da &
                            + (   qv(:,:,ke,nnow) + qc(:,:,ke,nnow) + qi(:,:,ke,nnow) &
                                + qr(:,:,ke,nnow) + qs(:,:,ke,nnow) ) * molmass_w
     ENDIF

   DO iig=1, ntracer
     IF ( ltracer(7,iig) == 1) THEN
       co2_s(:,:) = tracer(:,:,ke,nnow,iig) * molmass_ma(:,:,ke) / molmass_co2 &
                                            * ( p0(:,:,ke) + pp(:,:,ke,nnow) )
     ENDIF
   ENDDO
