...
route "${cblue}>> configure_clm${cnormal}"
  ...
  LMASK="navy"
  if [[ $cplscheme == "true" ]] ; then ; schemeFlag+=" -DCPL_SCHEME_F " ; fi
  ...
  comment "   sed comflg to clm Makefiles"
    sed -i "s@__comflg__@$optComp $cplInc -Duse_libMPI -Duse_netCDF -Duse_comm_MPI1 -DVERBOSE -DTREAT_OVERLAY -DNO_C_SIZEOF $cplFlag $schemeFlag@" $file >> $log_file 2>> $err_file
  check
    sed -i "s@__comflg__@$optComp $cplInc -WF,-Duse_libMPI -WF,-Duse_netCDF -WF,-Duse_comm_MPI1 -WF,-DVERBOSE -WF,-DTREAT_OVERLAY  -WF,$cplFlag -WF,$schemeFlag@" $file2 >> $log_file 2>> $err_file
  check
  ...
  comment "   sed ldflg to machine definition"
    sed -i "s@__slibs__@ -Wl,-allow-multiple-definition -L$lapackPath/lib -L/bgsys/local/lib -llapack -lesslbg  -L$liboas -L$libpsmile -L$ncdfPath/lib/ -lnetcdf @" $file3 >> $log_file 2>> $err_file
  check
