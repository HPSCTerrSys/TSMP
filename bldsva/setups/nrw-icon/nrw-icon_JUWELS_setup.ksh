...
#  defaultFDOAS="$rootdir/bldsva/setups/icon-ccs-s/input"
#  defaultFDCLM="$rootdir/bldsva/setups/icon-ccs-s/input"
#  defaultFDPFL="$rootdir/bldsva/setups/icon-ccs-s/input"
#  defaultFDICON="$rootdir/bldsva/setups/icon-ccs-s/input"

  defaultFDOAS="/p/project/cslts/poll1/data/nrw/oas/"
  defaultFDCLM="/p/project/cslts/poll1/data/nrw/clm/"
  defaultFDPFL="/p/project/cslts/poll1/data/nrw/pfl/"
#  defaultFDICON="/p/project/cslts/poll1/data/nrw/icon/"
 defaultFDICON="/p/project/cslts/poll1/data/nrw/icon_small/"

  defaultNLICON="$rootdir/bldsva/setups/nrw-icon/icon_master.namelist $rootdir/bldsva/setups/nrw-icon/NAMELIST_icon"
  defaultNLCLM=$rootdir/bldsva/setups/nrw-icon/lnd.stdin 
  defaultNLPFL=$rootdir/bldsva/setups/nrw-icon/rurlaf.tcl 

#  gicon=$(find $defaultFDICON -name torus_\*.nc)
#  gicon=$defaultFDICON"/pheno_juelich_10km_0078m.nc"
#  gicon=$defaultFDICON"/pheno_juelich_10km_bid12_0078m.nc"
#  gicon=$defaultFDICON"/grid_nrw_1247m.nc"
  gicon=$defaultFDICON"/grid_nrw_small_1247m.nc"
  ...
  defaultNppn=48 #24
  defaultICONProc=142 #92 #1 #140 #344 #1456
  defaultCLMProcX=2 #6
  defaultCLMProcY=2 #6
  defaultPFLProcX=6 #8
  defaultPFLProcY=6 #8

  defaultStartDate="2013-04-24 00" #"2013-04-24 00" #"2008-05-08 06"
  defaultInitDate=$defaultStartDate #"2013-04-24 00" #"2008-05-08 06"
  scriptDate="$(date -d "${defaultInitDate[0]} 0 hours" "+%Y%m%d%H")"
  defaultRunhours=1 #18 # 24
...
#  module load GCC/9.3.0  ParaStationMPI/5.4.7-1 CDO/1.9.8 NCO/4.9.5 #SPo
  rmp_folder="$rootdir/bldsva/setups/nrw-icon/create_remap"
  rm -rf $rmp_folder
  mkdir $rmp_folder
...
EOF
  ...
  res="${res_x}x${res_y}"
  dt_clm=3 #300 #45 #3000 #45 # in seconds
  ...
  cplfreq1=30 #3000 #450 # 3000 #450  # icon <-> clm, in tenths of seconds
...
route "${cyellow}>> finalizeSetup${cnormal}"
  ...
  comment "  sed icon "
    ...
    sed "s,__grid_icon_bldsva__,$gnicon," -i $rundir/NAMELIST_icon >> $log_file 2>> $err_file 
    sed "s,__dir_icon_bldsva__,$defaultFDICON," -i $rundir/NAMELIST_icon >> $log_file 2>> $err_file
  check
  ...
  comment "  link neccasary files"
    ...
    ln -s $defaultFDICON"/forcing/init_"${gnicon}"_"$fdSD".nc" $rundir"/dwdFG_R2B11_DOM01.nc" >> $log_file 2>> $err_file
    ln -s $defaultFDICON"/forcing/init_"${gnicon}"_"$fdSD".nc" $rundir"/ifs2icon_R2B11_DOM01.nc" >> $log_file 2>> $err_file
    ln -s $defaultFDICON"/dict.icon.nrw" $rundir/ >> $log_file 2>> $err_file
