...
initSetup(){
  if [[ $forcingdir_clm == "" ]] ; then ;forcingdir_clm="/work/slts/slts00/tsmp/TerrSysMPdb/idealized/2400";fi
  if [[ $forcingdir_cos == "" ]] ; then ;forcingdir_cos="";fi
  if [[ $forcingdir_oas == "" ]] ; then ;forcingdir_oas="/work/slts/slts00/tsmp/TerrSysMPdb/idealized/2400/oasis3";fi
  if [[ $forcingdir_pfl == "" ]] ; then ;forcingdir_pfl="";fi


  if [[ $namelist_clm == "" ]] ; then ; namelist_clm=$rootdir/bldsva/setups/ideal24001200/lnd.stdin ; fi
  if [[ $namelist_cos == "" ]] ; then ; namelist_cos=$rootdir/bldsva/setups/ideal24001200/lmrun_uc ; fi
  if [[ $namelist_pfl == "" ]] ; then ; namelist_pfl=$rootdir/bldsva/setups/ideal24001200/coup_oas.tcl ; fi
