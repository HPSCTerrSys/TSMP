...
IF ( masterproc ) THEN
 CALL prism_create_couplcomm(1, kl_comm, c_comm, nerror)
else
  CALL prism_create_couplcomm(MPI_UNDEFINED, kl_comm, c_comm, nerror)
endif
...
!        CALL prism_def_partition_proto( igrid, igparal, nerror )
 !       IF( nerror /= PRISM_Success )   CALL prism_abort_proto (ncomp_id, 'oas_clm_define','Failure in prism_def_partition' )
